...
/* ============================================================
   DIRAC BOLA/IDOR PARENT OWNER BINDING v127 - APPEND ONLY
   - Tidak mengubah endpoint, login, daftar, hash, payment gateway,
     email template, A2F/MFA/passkey, logout, atau auto-logout.
   - Mengikat akses child-table order_items/domain_order_items ke parent order
     yang sudah terbukti berasal dari customer_id milik session backend.
   - Fail-safe: hanya block saat mismatch high-confidence; selain itu tetap menjaga
     kompatibilitas website agar fitur lama tidak rusak.
   ============================================================ */

const DIRAC_BOLA_IDOR_PARENT_OWNER_BINDING_PATCH_V127 = 'dirac-bola-idor-parent-owner-binding-v127';

let diracBolaIdorAsyncLocalV127 = null;
try {
  const asyncHooks = require('async_hooks');
  if (asyncHooks && typeof asyncHooks.AsyncLocalStorage === 'function') {
    diracBolaIdorAsyncLocalV127 = new asyncHooks.AsyncLocalStorage();
  }
} catch (_) {}

try {
  const __diracBolaIdorV127PreviousHandler = module.exports;
  if (typeof __diracBolaIdorV127PreviousHandler === 'function' && !__diracBolaIdorV127PreviousHandler.__diracBolaIdorV127Wrapped) {
    module.exports = async function diracBolaIdorParentOwnerBindingContextWrapperV127(req, res) {
      const context = diracBolaIdorV127BuildRequestContext(req);
      if (diracBolaIdorAsyncLocalV127 && typeof diracBolaIdorAsyncLocalV127.run === 'function') {
        return diracBolaIdorAsyncLocalV127.run(context, () => __diracBolaIdorV127PreviousHandler(req, res));
      }
      try { if (req && typeof req === 'object') req.__diracBolaIdorContextV127 = context; } catch (_) {}
      return __diracBolaIdorV127PreviousHandler(req, res);
    };
    Object.defineProperty(module.exports, '__diracBolaIdorV127Wrapped', { value: true, enumerable: false });
  }
} catch (_) {}
...
try {
  ...
  if (__diracBolaIdorV127OriginalSupabaseFetch && !__diracBolaIdorV127OriginalSupabaseFetch.__diracBolaIdorV127Wrapped) {
    supabaseFetch = async function supabaseFetchBolaIdorParentOwnerBindingV127(path, options = {}) {
      const before = diracBolaIdorV127InspectBeforeFetch(path, options);
      ...
      diracBolaIdorV127LearnFromFetch(path, options, result);
      return result;
    };
...
function diracBolaIdorV127BuildRequestContext(req) {
  const query = req && req.query && typeof req.query === 'object' ? req.query : {};
  const rawAction = String(query.action || '').trim();
  return {
    patch: DIRAC_BOLA_IDOR_PARENT_OWNER_BINDING_PATCH_V127,
    action: diracBolaIdorV127NormalizeAction(rawAction),
    raw_action: rawAction.slice(0, 120),
    method: String(req && req.method || '').toUpperCase().slice(0, 12),
    allowedCustomerIdsV127: [],
    allowedOrderIdsV127: [],
    allowedDomainOrderIdsV127: [],
    started_at: Date.now()
  };
}
...
function diracBolaIdorV127CurrentContext() {
  try {
    if (diracBolaIdorAsyncLocalV127 && typeof diracBolaIdorAsyncLocalV127.getStore === 'function') {
      const store = diracBolaIdorAsyncLocalV127.getStore();
      if (store && typeof store === 'object') return store;
    }
  } catch (_) {}
  try {
    if (typeof diracBolaIdorV126CurrentContext === 'function') {
      const ctx = diracBolaIdorV126CurrentContext();
      if (ctx && typeof ctx === 'object') return ctx;
    }
  } catch (_) {}
...
function diracBolaIdorV127InspectBeforeFetch(path, options = {}) {
  try {
    if (!options || options.auth !== 'service') return { ok: true };
    if (diracBolaIdorV127EnvTrue('DIRAC_BOLA_IDOR_PARENT_BINDING_DISABLED', false)) return { ok: true };
    ...
    if (!diracBolaIdorV127IsRelevantOwnedTable(table)) return { ok: true };

    const ctx = diracBolaIdorV127CurrentContext() || {};
    const action = diracBolaIdorV127NormalizeAction(ctx.action || '');
    if (!action || !diracBolaIdorV127IsUserDataAction(action)) return { ok: true };
    if (diracBolaIdorV127IsUntouchedFlowAction(action)) return { ok: true };

    const ownerDecision = diracBolaIdorV127InspectRequestedCustomerIds(ctx, table, method, action, rawPath, options.body);
    if (ownerDecision && !ownerDecision.ok) return ownerDecision;

    const childDecision = diracBolaIdorV127InspectRequestedChildOrderIds(ctx, table, method, action, rawPath, options.body);
    if (childDecision && !childDecision.ok) return childDecision;
...
function diracBolaIdorV127InspectRequestedCustomerIds(ctx, table, method, action, rawPath, body) {
  const requestedCustomerIds = diracBolaIdorV127ExtractColumnValues(rawPath, body, 'customer_id')
    .filter(diracBolaIdorV127LooksLikeUuid);
  if (!requestedCustomerIds.length) return { ok: true };

  const allowedCustomerIds = diracBolaIdorV127AllowedCustomerIds(ctx);
  if (!allowedCustomerIds.length) return { ok: true, reason: 'trusted_customer_owner_not_yet_available' };

  const denied = requestedCustomerIds.filter((id) => !allowedCustomerIds.includes(id));
  ...
  return {
    ...
    allowed_count: allowedCustomerIds.length,
    patch: DIRAC_BOLA_IDOR_PARENT_OWNER_BINDING_PATCH_V127
  };
}

function diracBolaIdorV127InspectRequestedChildOrderIds(ctx, table, method, action, rawPath, body) {
  if (!/^(GET|HEAD|PATCH|PUT|DELETE)$/i.test(method)) return { ok: true };
  const normalizedTable = String(table || '').toLowerCase();
  if (normalizedTable !== 'order_items' && normalizedTable !== 'domain_order_items') return { ok: true };

  const requestedOrderIds = diracBolaIdorV127ExtractColumnValues(rawPath, body, 'order_id')
    .filter(diracBolaIdorV127LooksLikeUuid);
  if (!requestedOrderIds.length) return { ok: true };

  const allowed = normalizedTable === 'domain_order_items'
    ? diracBolaIdorV127AllowedDomainOrderIds(ctx)
    : diracBolaIdorV127AllowedOrderIds(ctx);

  if (!allowed.length) return { ok: true, reason: 'trusted_parent_orders_not_yet_available' };

  const denied = requestedOrderIds.filter((id) => !allowed.includes(id));
  if (!denied.length) return { ok: true };
  ...
  return {
    ...
    method,
    action,
    reason: 'blocked_order_id_not_bound_to_authenticated_customer',
    requested_count: requestedOrderIds.length,
    allowed_count: allowed.length,
    patch: DIRAC_BOLA_IDOR_PARENT_OWNER_BINDING_PATCH_V127
  };
}

function diracBolaIdorV127LearnFromFetch(path, options = {}, result) {
  try {
    if (!options || options.auth !== 'service' || !result || !result.ok) return;
    const rawPath = String(path || '').trim();
    if (!rawPath.startsWith('/rest/v1/')) return;
    ...
    if (table === 'security_customer_auth_links') {
      diracBolaIdorV127LearnCustomerIdsFromAuthLinks(ctx, rawPath, rows);
      return;
    }

    if (table === 'orders' || table === 'domain_orders') {
      diracBolaIdorV127LearnParentOrderIds(ctx, table, rows);
    }
  } catch (_) {}
}

function diracBolaIdorV127LearnCustomerIdsFromAuthLinks(ctx, rawPath, rows) {
  try {
    if (!diracBolaIdorV127PathHasColumnFilter(rawPath, 'auth_user_id')) return;
    const ids = rows
      .filter((row) => row && String(row.link_status || '').toLowerCase() === 'active')
      .map((row) => String(row.customer_id || '').trim())
      .filter(diracBolaIdorV127LooksLikeUuid);
    if (!ids.length) return;
    ctx.allowedCustomerIdsV127 = diracBolaIdorV127MergeUuidList(ctx.allowedCustomerIdsV127, ids, 25);

    try {
      const v126Ids = Array.isArray(ctx.allowedCustomerIdsV126) ? ctx.allowedCustomerIdsV126 : [];
      ctx.allowedCustomerIdsV126 = diracBolaIdorV127MergeUuidList(v126Ids, ids, 25);
    } catch (_) {}
  } catch (_) {}
}

function diracBolaIdorV127LearnParentOrderIds(ctx, table, rows) {
  try {
    const allowedCustomers = diracBolaIdorV127AllowedCustomerIds(ctx);
    if (!allowedCustomers.length) return;
    ...
    rows.forEach((row) => {
      ...
      if (!diracBolaIdorV127LooksLikeUuid(id)) return;
      if (!diracBolaIdorV127LooksLikeUuid(customerId)) return;
      if (!allowedCustomers.includes(customerId)) return;
      allowedOrderIds.push(id);
    });
    if (!allowedOrderIds.length) return;

    if (table === 'domain_orders') {
      ctx.allowedDomainOrderIdsV127 = diracBolaIdorV127MergeUuidList(ctx.allowedDomainOrderIdsV127, allowedOrderIds, 250);
    } else {
      ctx.allowedOrderIdsV127 = diracBolaIdorV127MergeUuidList(ctx.allowedOrderIdsV127, allowedOrderIds, 250);
    }
  } catch (_) {}
}
...
function diracBolaIdorV127AllowedCustomerIds(ctx) {
  try {
    const ids = [];
    if (Array.isArray(ctx && ctx.allowedCustomerIdsV127)) ids.push(...ctx.allowedCustomerIdsV127);
    if (Array.isArray(ctx && ctx.allowedCustomerIdsV126)) ids.push(...ctx.allowedCustomerIdsV126);
    return diracBolaIdorV127MergeUuidList([], ids, 25);
  } catch (_) { return []; }
}

function diracBolaIdorV127AllowedOrderIds(ctx) {
  try { return diracBolaIdorV127MergeUuidList([], Array.isArray(ctx && ctx.allowedOrderIdsV127) ? ctx.allowedOrderIdsV127 : [], 250); } catch (_) { return []; }
}

function diracBolaIdorV127AllowedDomainOrderIds(ctx) {
  try { return diracBolaIdorV127MergeUuidList([], Array.isArray(ctx && ctx.allowedDomainOrderIdsV127) ? ctx.allowedDomainOrderIdsV127 : [], 250); } catch (_) { return []; }
}

function diracBolaIdorV127ExtractColumnValues(path, body, column) {
  const values = [];
  values.push(...diracBolaIdorV127ExtractColumnValuesFromPath(path, column));
  values.push(...diracBolaIdorV127ExtractColumnValuesFromBody(body, column));
  return Array.from(new Set(values.map((value) => String(value || '').trim()).filter(Boolean))).slice(0, 300);
}

function diracBolaIdorV127ExtractColumnValuesFromBody(body, column) {
  const col = String(column || '').trim();
  if (!col || body === undefined || body === null) return [];
  const rows = Array.isArray(body) ? body : [body];
  const out = [];
  for (const row of rows) {
    ...
    const value = row[col];
    if (Array.isArray(value)) {
      value.forEach((item) => out.push(String(item || '').trim()));
    } else {
      out.push(String(value || '').trim());
    }
  }
...
function diracBolaIdorV127ExtractColumnValuesFromPath(path, column) {
  const col = String(column || '').trim().toLowerCase();
  if (!col) return [];
  const decoded = diracBolaIdorV127SafeDecode(path);
  const query = decoded.split('?')[1] || '';
  if (!query) return [];
  ...
  for (const part of query.split('&')) {
    ...
    const key = part.slice(0, idx).toLowerCase();
    const value = part.slice(idx + 1);
    if (key === col) out.push(...diracBolaIdorV127ParsePostgrestFilterValue(value));
    if (key === 'or') out.push(...diracBolaIdorV127ParseOrFilterValues(value, col));
  }
  return out;
}

function diracBolaIdorV127ParsePostgrestFilterValue(value) {
  const raw = String(value || '').trim();
  if (!raw) return [];
  const lower = raw.toLowerCase();
  if (lower.startsWith('eq.')) return [raw.slice(3).replace(/^"|"$/g, '').trim()].filter(Boolean);
  if (lower.startsWith('in.')) {
    const inner = raw.slice(3).replace(/^\(/, '').replace(/\)$/, '');
    return inner.split(',').map((item) => item.replace(/^"|"$/g, '').trim()).filter(Boolean);
  }
...
function diracBolaIdorV127ParseOrFilterValues(value, column) {
  const raw = String(value || '').replace(/^\(/, '').replace(/\)$/, '');
  const col = String(column || '').toLowerCase();
  const out = [];
  for (const item of raw.split(',')) {
    const text = item.trim();
    const lower = text.toLowerCase();
    if (lower.startsWith(col + '.eq.')) out.push(text.slice(col.length + 4).replace(/^"|"$/g, '').trim());
    if (lower.startsWith(col + '.in.')) out.push(...diracBolaIdorV127ParsePostgrestFilterValue(text.slice(col.length + 1)));
  }
  return out.filter(Boolean);
}

function diracBolaIdorV127PathHasColumnFilter(path, column) {
  try {
    if (typeof diracBolaIdorV126PathHasColumnFilter === 'function') return diracBolaIdorV126PathHasColumnFilter(path, column);
  } catch (_) {}
  return diracBolaIdorV127ExtractColumnValuesFromPath(path, column).length > 0;
}

function diracBolaIdorV127IsRelevantOwnedTable(table) {
  return /^(orders|order_items|domain_orders|domain_order_items|payment_transactions|security_customer_sessions|security_customer_settings|security_customer_recovery_codes|security_customer_auth_links|security_customer_password_hashes|customer_security_events|domain_passkeys|security_customer_login_logs|security_customer_account_requests|payment_gateway_events|customers)$/i.test(String(table || ''));
}
...
function diracBolaIdorV127IsUntouchedFlowAction(action) {
  return /login|register|logout|payment|pay|midtrans|ipaymu|webhook|callback|notification|checkout|mfa|a2f|passkey|password|hash|email|mail|csrf|token|session|recovery|admin/i.test(String(action || ''));
}
...
function diracBolaIdorV127BlockedSupabaseResult(decision) {
  return {
    ...
    data: {
      ok: false,
      code: 'BOLA_IDOR_PARENT_OWNER_MISMATCH',
      message: 'Permintaan ditolak oleh sistem keamanan.',
      reason: decision && decision.reason || 'object_not_bound_to_session_owner'
    },
    error: 'BOLA_IDOR_PARENT_OWNER_MISMATCH',
    diracSecurityThreat: {
      detected: true,
      kind: 'bola_idor_parent_owner_mismatch',
      table: decision && decision.table || null,
      method: decision && decision.method || null,
      action: decision && decision.action || null,
      patch: DIRAC_BOLA_IDOR_PARENT_OWNER_BINDING_PATCH_V127
...
function diracBolaIdorV127MergeUuidList(existing, incoming, max) {
  const values = [];
  if (Array.isArray(existing)) values.push(...existing);
  if (Array.isArray(incoming)) values.push(...incoming);
  return Array.from(new Set(values.map((value) => String(value || '').trim()).filter(diracBolaIdorV127LooksLikeUuid))).slice(0, Math.max(1, Number(max || 100) || 100));
}

function diracBolaIdorV127SafeDecode(value) {
  try {
    if (typeof diracBolaIdorV126SafeDecode === 'function') return diracBolaIdorV126SafeDecode(value);
  } catch (_) {}
  let out = String(value || '');
  for (let i = 0; i < 2; i += 1) {
    try {
...
function diracBolaIdorV127LogDecision(decision) {
  try {
    if (diracBolaIdorV127EnvTrue('DIRAC_BOLA_IDOR_PARENT_BINDING_SILENT', false)) return;
    const key = [decision.table, decision.method, decision.action, decision.reason, decision.block ? 'block' : 'warn'].join('|');
    const store = diracBolaIdorV127SeenStore();
    const current = Number(store.get(key) || 0);
    const max = Math.max(1, Number(process.env.DIRAC_BOLA_IDOR_PARENT_BINDING_LOG_LIMIT || 5) || 5);
    if (current >= max) return;
    store.set(key, current + 1);
    console.warn('[dirac-bola-idor-parent-owner-binding]', {
      patch: DIRAC_BOLA_IDOR_PARENT_OWNER_BINDING_PATCH_V127,
      mode: decision.block ? 'blocked' : 'monitor',
      reason: decision.reason,
      table: decision.table,

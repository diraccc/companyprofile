...
/* ============================================================
   DIRAC BOLA/IDOR GLOBAL HARD-BAN v128 - APPEND ONLY
   Tujuan:
   - Menutup percobaan BOLA/IDOR high-confidence dari link/query/API client
     seperti Termux/Postman dengan hard-ban yang memakai sistem global-ban lama.
   - Menambah parent-owner binding untuk order_items/domain_order_items saat
     request service-role memakai order_id/domain_order_id.
   - Tetap tidak mengubah endpoint, login, register, logout, auto-logout 5 menit,
     hash, payment gateway, email template, A2F/MFA/passkey, atau tampilan.
   - Fail-safe untuk alur normal: ban hanya saat owner mismatch terbukti jelas.
   ============================================================ */
...
try {
  const asyncHooksV128 = require('async_hooks');
  if (asyncHooksV128 && typeof asyncHooksV128.AsyncLocalStorage === 'function') {
    diracBolaIdorAsyncLocalV128 = new asyncHooksV128.AsyncLocalStorage();
  }
} catch (_) {}
...
try {
  ...
  if (__diracBolaIdorV128OriginalSupabaseFetch && !__diracBolaIdorV128OriginalSupabaseFetch.__diracBolaIdorV128Wrapped) {
    supabaseFetch = async function supabaseFetchBolaIdorGlobalHardBanV128(path, options = {}) {
      const decision = await diracBolaIdorV128InspectSupabaseAccess(path, options).catch(() => ({ ok: true }));
      ...
      diracBolaIdorV128LearnTrustedOwners(path, options, result);
      return result;
    };
    Object.defineProperty(supabaseFetch, '__diracBolaIdorV128Wrapped', { value: true, enumerable: false });
  }
} catch (_) {}
  ...
  if (typeof __diracBolaIdorV128PreviousHandler === 'function' && !__diracBolaIdorV128PreviousHandler.__diracBolaIdorV128Wrapped) {
    module.exports = async function diracBolaIdorGlobalHardBanWrapperV128(req, res) {
      ...
      const run = async () => {
        ...
        const decision = await diracBolaIdorV128InspectHttpRequest(req).catch(() => ({ ok: true }));
        if (decision && decision.warn) diracBolaIdorV128LogDecision(decision);
        if (decision && decision.block) {
      ...
      if (diracBolaIdorAsyncLocalV128 && typeof diracBolaIdorAsyncLocalV128.run === 'function') {
        return diracBolaIdorAsyncLocalV128.run(context, run);
      }
...
/* ============================================================
   DIRAC BACKEND BOLA/IDOR OBJECT-OWNER VERIFIER v134 - APPEND ONLY
   Scope:
   - Backend-only hardening for service-role access on protected customer data.
   - Does not change endpoints, login/register/logout, payment, email template,
     hash/password, A2F/MFA/passkey, token issuance, or 5-minute idle logout.
   - Does not re-enable the v132 dashboard hard-lock that caused dashboard bounce.
   Goal:
   - When a protected-data backend query uses an object id without an owner filter,
     verify the object's real customer owner before allowing the service-role call.
   - Re-resolve request owner from the active request context when earlier wrappers
     have not learned allowedCustomerIds yet.
   ============================================================ */

const DIRAC_BACKEND_BOLA_OBJECT_OWNER_VERIFIER_PATCH_V134 = 'dirac-backend-bola-object-owner-verifier-v134';

try {
  const __diracBackendV134PreviousSupabaseFetch = typeof supabaseFetch === 'function' ? supabaseFetch : null;
  if (__diracBackendV134PreviousSupabaseFetch && !__diracBackendV134PreviousSupabaseFetch.__diracBackendBolaObjectOwnerVerifierV134) {
    supabaseFetch = async function supabaseFetchBackendBolaObjectOwnerVerifierV134(path, options = {}) {
      const decision = await diracBackendV134InspectServiceRoleObjectAccess(path, options).catch(() => ({ ok: true }));
      if (decision && decision.block) return diracBackendV134BlockedSupabaseResult(decision);
      return __diracBackendV134PreviousSupabaseFetch(path, options);
    };
    Object.defineProperty(supabaseFetch, '__diracBackendBolaObjectOwnerVerifierV134', { value: true, enumerable: false });
  }
} catch (_) {}

try {
  const __diracBackendV134PreviousHandler = module.exports;
  if (typeof __diracBackendV134PreviousHandler === 'function' && !__diracBackendV134PreviousHandler.__diracBackendBolaObjectOwnerVerifierV134) {
    module.exports = async function diracBackendBolaObjectOwnerVerifierWrapperV134(req, res) {
      try { if (res && typeof res.setHeader === 'function') res.setHeader('X-Dirac-Backend-Bola-Object-Owner-Verifier', DIRAC_BACKEND_BOLA_OBJECT_OWNER_VERIFIER_PATCH_V134); } catch (_) {}
      return __diracBackendV134PreviousHandler(req, res);
    };
    Object.defineProperty(module.exports, '__diracBackendBolaObjectOwnerVerifierV134', { value: true, enumerable: false });
  }
} catch (_) {}

async function diracBackendV134InspectServiceRoleObjectAccess(path, options = {}) {
  if (diracBackendV134EnvTrue('DIRAC_BACKEND_BOLA_OBJECT_OWNER_VERIFIER_DISABLED', false)) return { ok: true };
  if (!options || options.auth !== 'service') return { ok: true };

  const rawPath = String(path || '').trim();
  if (!rawPath || !rawPath.startsWith('/rest/v1/')) return { ok: true };

  const ctx = diracBackendV134CurrentContext() || {};
  const action = diracBackendV134NormalizeAction(ctx.action || '');
  const method = String(options.method || 'GET').toUpperCase();
  if (!diracBackendV134ShouldProtectAction(action, method, ctx.req)) return { ok: true };

  const table = diracBackendV134ExtractRestTable(rawPath);
  const policy = diracBackendV134OwnedTablePolicy(table);
  if (!policy) return { ok: true };

  const ids = diracBackendV134CollectIdsFromSupabase(rawPath, options.body);
  if (!ids.length) return { ok: true };

  const owner = await diracBackendV134ResolveOwner(ctx).catch(() => null);
  if (!owner || !owner.ok) {
    if (diracBackendV134IsDangerousOwnerlessWrite(method, ids)) {
      return diracBackendV134Decision('owner_unavailable_for_service_role_object_write', { table, method, action, status: 401 });
    }
    return { ok: true };
  }

  const customerMismatch = diracBackendV134FindCustomerMismatch(ids, owner.customerIds || []);
  if (customerMismatch) {
    return diracBackendV134Decision('service_role_customer_id_not_bound_to_authenticated_owner', {
      table,
      method,
      action,
      requested_count: customerMismatch.requestedCount,
  ...
  const authMismatch = diracBackendV134FindAuthUserMismatch(ids, owner.authUserId || '');
  if (authMismatch) {
    return diracBackendV134Decision('service_role_auth_user_id_not_bound_to_authenticated_user', {
      table,
      method,
      action,
      requested_count: authMismatch.requestedCount,
  ...
  if (!/^(GET|HEAD|PATCH|PUT|DELETE)$/i.test(method)) return { ok: true };
  if (diracBackendV134HasOwnerScope(rawPath, options.body, policy.ownerColumns)) return { ok: true };

  const allowed = Array.from(new Set((owner.customerIds || []).map((id) => String(id || '').trim()).filter(diracBackendV134LooksLikeUuid))).slice(0, 25);
  if (!allowed.length) return { ok: true };

  const directObjectIds = diracBackendV134DirectObjectIdsForTable(table, ids);
  if (directObjectIds.length) {
    const owners = await diracBackendV134ResolveKnownObjectOwners(directObjectIds, table).catch(() => []);
    const foreign = owners.filter((row) => row && row.customer_id && !allowed.includes(String(row.customer_id || '').trim()));
    if (foreign.length) {
      return diracBackendV134Decision('service_role_object_id_not_bound_to_authenticated_owner', {
        table,
        method,
        action,
        requested_count: directObjectIds.length,
  ...
  if (diracBackendV134IsChildOrderTable(table)) {
    const parentIds = diracBackendV134ChildOrderIds(table, ids);
    if (parentIds.length) {
      const owners = await diracBackendV134ResolveChildParentOwners(table, parentIds).catch(() => []);
      const foreign = owners.filter((row) => row && row.customer_id && !allowed.includes(String(row.customer_id || '').trim()));
      if (foreign.length) {
        return diracBackendV134Decision('service_role_child_parent_not_bound_to_authenticated_owner', {
          table,
          method,
          action,
          requested_count: parentIds.length,
...
function diracBackendV134CurrentContext() {
  try { if (typeof diracBolaIdorV132CurrentContext === 'function') return diracBolaIdorV132CurrentContext(); } catch (_) {}
  try { if (typeof diracBolaIdorV128CurrentContext === 'function') return diracBolaIdorV128CurrentContext(); } catch (_) {}
  try { if (typeof diracBolaIdorV126CurrentContext === 'function') return diracBolaIdorV126CurrentContext(); } catch (_) {}
  return null;
}

async function diracBackendV134ResolveOwner(ctx) {
  const current = ctx && typeof ctx === 'object' ? ctx : {};
  const existing = {
    ok: true,
    authUserId: diracBackendV134AuthUserIdFromContext(current),
    customerIds: diracBackendV134AllowedCustomerIds(current)
  };
  if (diracBackendV134LooksLikeUuid(existing.authUserId) && existing.customerIds.length) return existing;

  if (current.req && typeof diracBolaIdorV132ResolveRequestOwner === 'function') {
    const owner = await diracBolaIdorV132ResolveRequestOwner(current.req).catch(() => null);
    if (owner && owner.ok) {
      try {
        current.ownerResolvedV134 = owner;
        current.ownerResolvedV128 = current.ownerResolvedV128 || owner;
        current.allowedCustomerIdsV128 = Array.from(new Set((current.allowedCustomerIdsV128 || []).concat(owner.customerIds || []))).slice(0, 25);
      } catch (_) {}
      return {
        ok: true,
        authUserId: String(owner.authUserId || '').trim(),
        customerIds: Array.from(new Set((owner.customerIds || []).map((id) => String(id || '').trim()).filter(diracBackendV134LooksLikeUuid))).slice(0, 25)
      };
    }
  }

  if (diracBackendV134LooksLikeUuid(existing.authUserId) || existing.customerIds.length) return existing;
  return { ok: false, authUserId: '', customerIds: [] };
}

function diracBackendV134ShouldProtectAction(action, method, req) {
  const clean = String(action || '').toLowerCase();
  const upper = String(method || 'GET').toUpperCase();
  if (!clean) return false;
  try { if (typeof diracBolaIdorV132NeverTouchAction === 'function' && diracBolaIdorV132NeverTouchAction(clean)) return false; } catch (_) {}
  try { if (typeof diracBolaIdorV128ShouldSkipAction === 'function' && diracBolaIdorV128ShouldSkipAction(clean, upper, req)) return false; } catch (_) {}
  try { if (typeof diracBolaIdorV132ShouldProtectDataAction === 'function' && diracBolaIdorV132ShouldProtectDataAction(clean, upper)) return true; } catch (_) {}
  return /^(domain_dashboard_me|domain_orders|my_orders|pesanan|pesanan_saya|customer_orders|orders_saya|my_invoices|invoice_saya)$/i.test(clean)
    || (/^customer_security_(status|overview|features_bundle_v2|features_bundle_v3|sessions|devices|events|blocks)$/i.test(clean) && upper === 'GET');
}

function diracBackendV134IsDangerousOwnerlessWrite(method, ids) {
  if (!/^(PATCH|PUT|DELETE)$/i.test(String(method || ''))) return false;
  return (ids || []).some((item) => item && diracBackendV134LooksLikeUuid(item.value) && /^(id|customer_id|auth_user_id|user_id|owner_user_id|order_id|domain_order_id|session_id|recovery_code_id|transaction_id|payment_transaction_id)$/i.test(item.key));
}

function diracBackendV134AllowedCustomerIds(ctx) {
  try { if (typeof diracBolaIdorV132AllowedCustomerIds === 'function') return diracBolaIdorV132AllowedCustomerIds(ctx); } catch (_) {}
  try { if (typeof diracBolaIdorV128AllowedCustomerIds === 'function') return diracBolaIdorV128AllowedCustomerIds(ctx); } catch (_) {}
  return [];
}

function diracBackendV134AuthUserIdFromContext(ctx) {
  try { if (typeof diracBolaIdorV132AuthUserIdFromContext === 'function') return diracBolaIdorV132AuthUserIdFromContext(ctx); } catch (_) {}
  const text = String(ctx && ctx.user && ctx.user.id || ctx && ctx.authUserId || '').trim();
  return diracBackendV134LooksLikeUuid(text) ? text : '';
}

function diracBackendV134OwnedTablePolicy(table) {
  try { if (typeof diracBolaIdorV132OwnedTablePolicy === 'function') return diracBolaIdorV132OwnedTablePolicy(table); } catch (_) {}
  return null;
}

function diracBackendV134HasOwnerScope(path, body, ownerColumns) {
  try { if (typeof diracBolaIdorV132HasOwnerScope === 'function') return diracBolaIdorV132HasOwnerScope(path, body, ownerColumns); } catch (_) {}
  return false;
}

function diracBackendV134DirectObjectIdsForTable(table, ids) {
  try { if (typeof diracBolaIdorV128DirectObjectIdsForTable === 'function') return diracBolaIdorV128DirectObjectIdsForTable(table, ids); } catch (_) {}
  const clean = String(table || '').toLowerCase();
  const directKeys = diracBackendV134IsChildOrderTable(clean) ? /^(id)$/i : /^(id|order_id|domain_order_id|session_id|recovery_code_id|transaction_id|payment_transaction_id)$/i;
  return Array.from(new Set((ids || [])
    .filter((item) => item && directKeys.test(item.key))
    .map((item) => String(item.value || '').trim())
    .filter(diracBackendV134LooksLikeUuid))).slice(0, 40);
}

function diracBackendV134ChildOrderIds(table, ids) {
  try { if (typeof diracBolaIdorV128ChildOrderIds === 'function') return diracBolaIdorV128ChildOrderIds(table, ids); } catch (_) {}
  const clean = String(table || '').toLowerCase();
  const wanted = clean === 'domain_order_items' ? /^(order_id|domain_order_id)$/i : /^order_id$/i;
  return Array.from(new Set((ids || [])
    .filter((item) => item && wanted.test(item.key))
    .map((item) => String(item.value || '').trim())
    .filter(diracBackendV134LooksLikeUuid))).slice(0, 80);
}

function diracBackendV134ResolveKnownObjectOwners(ids, table) {
  if (typeof diracBolaIdorV128ResolveKnownObjectOwners === 'function') return diracBolaIdorV128ResolveKnownObjectOwners(ids, table);
  return Promise.resolve([]);
}

function diracBackendV134ResolveChildParentOwners(table, parentIds) {
  if (typeof diracBolaIdorV128ResolveChildParentOwners === 'function') return diracBolaIdorV128ResolveChildParentOwners(table, parentIds);
  return Promise.resolve([]);
}

function diracBackendV134IsChildOrderTable(table) {
  try { if (typeof diracBolaIdorV128IsChildOrderTable === 'function') return diracBolaIdorV128IsChildOrderTable(table); } catch (_) {}
  return /^(order_items|domain_order_items)$/i.test(String(table || ''));
}

function diracBackendV134CollectIdsFromSupabase(path, body) {
...
function diracBackendV134FindCustomerMismatch(ids, allowedCustomerIds) {
  try { if (typeof diracBolaIdorV132FindCustomerMismatch === 'function') return diracBolaIdorV132FindCustomerMismatch(ids, allowedCustomerIds); } catch (_) {}
  return null;
}

function diracBackendV134FindAuthUserMismatch(ids, authUserId) {
  try { if (typeof diracBolaIdorV132FindAuthUserMismatch === 'function') return diracBolaIdorV132FindAuthUserMismatch(ids, authUserId); } catch (_) {}
  return null;
}

function diracBackendV134ExtractRestTable(path) {
  try { if (typeof diracBolaIdorV132ExtractRestTable === 'function') return diracBolaIdorV132ExtractRestTable(path); } catch (_) {}
  try { if (typeof diracBolaIdorV128ExtractRestTable === 'function') return diracBolaIdorV128ExtractRestTable(path); } catch (_) {}
  return '';
}

function diracBackendV134NormalizeAction(action) {
  try { if (typeof diracBolaIdorV132NormalizeAction === 'function') return diracBolaIdorV132NormalizeAction(action); } catch (_) {}
  try { if (typeof normalizeDomainAction === 'function') return normalizeDomainAction(action); } catch (_) {}
  return String(action || '').trim().toLowerCase().replace(/[\s-]+/g, '_').slice(0, 120);
}

function diracBackendV134LooksLikeUuid(value) {
  try { if (typeof diracBolaIdorV132LooksLikeUuid === 'function') return diracBolaIdorV132LooksLikeUuid(value); } catch (_) {}
  try { if (typeof customerSecurityLooksLikeUuid === 'function') return customerSecurityLooksLikeUuid(value); } catch (_) {}
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(value || '').trim());
}

function diracBackendV134Decision(reason, extra = {}) {
  return {
    ok: false,
    block: true,
    reason: diracBackendV134Small(reason || 'backend_bola_object_owner_blocked', 120),
    patch: DIRAC_BACKEND_BOLA_OBJECT_OWNER_VERIFIER_PATCH_V134,
    ...extra
  };
}

function diracBackendV134BlockedSupabaseResult(decision) {
  return {
    ok: false,
    status: Number(decision && decision.status || 403),
    statusText: 'Forbidden',
    data: {
      ok: false,
      code: 'BACKEND_BOLA_OBJECT_OWNER_BLOCKED',
      message: 'Permintaan ditolak oleh sistem keamanan.',
      reason: diracBackendV134Small(decision && decision.reason || 'object_owner_blocked', 120)
    },
    error: 'BACKEND_BOLA_OBJECT_OWNER_BLOCKED',
    diracSecurityThreat: {
      detected: true,
      kind: 'backend_bola_object_owner_blocked',
      table: decision && decision.table || null,
      method: decision && decision.method || null,
      action: decision && decision.action || null,
      patch: DIRAC_BACKEND_BOLA_OBJECT_OWNER_VERIFIER_PATCH_V134
...
function diracBackendV134EnvTrue(name, fallback) {
  const raw = process.env[String(name || '')];
  if (raw === undefined || raw === null || raw === '') return !!fallback;
  return /^(1|true|yes|on)$/i.test(String(raw).trim());
}

function diracBackendV134Small(value, max) {
  return String(value || '').replace(/[\u0000-\u001f\u007f<>]/g, '').slice(0, Math.max(1, Number(max || 120)));
}

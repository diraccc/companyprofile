...
const EXTERNAL_QA_MAX_BYTES = Math.max(1024, Number(process.env.EXTERNAL_QA_MAX_BYTES || 30 * 1024 * 1024));
...
let EXTERNAL_QA_CACHE = { signature: '', entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, direct: new Map(), error: null, loadedAt: 0, source: '', files: [] };
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Setelah katalog produk dicek, cari jawaban di QA split secara bertahap.
    // Sistem ini membaca /data/qa-1.json sampai /data/qa-50.json satu per satu,
    // dan langsung berhenti saat pertanyaan ditemukan agar Vercel tidak perlu memuat semua file besar sekaligus.
    const primaryExternalQa = externalQaFallbackReply(message, traceId, intent.name, startedAt, 'split-qa-after-product');
...
function buildExternalQaNoAnswerMessage() {
  return 'Maaf, saya belum punya jawaban untuk pertanyaan itu. Admin bisa menambahkan pertanyaan dan jawaban ke file /data/qa.json agar ke depan saya bisa menjawab otomatis.';
}
...
function externalQaStatus() {
  ...
  if (files.length) {
    return {
      source: files.map((file) => file.publicPath).join(', '),
      status: 'split-ready',
      files: files.map((file) => file.publicPath),
      entries: 'direct-exact-scan'
    };
  }
...
function findExternalQaAnswer(message) {
  ...
  // Prioritas 1: file split /data/qa-1.json sampai /data/qa-50.json.
  // Dibuat direct exact-match berbasis key JSON supaya tidak perlu parse seluruh jutaan QA ke memory.
  const splitHit = findExternalQaAnswerInSplitFiles(userNorm);
  if (splitHit) return splitHit;

  // Prioritas 2: kompatibilitas lama untuk /data/qa.json jika masih dipakai.
  const loaded = loadExternalQaEntries();
...
function findExternalQaAnswerInSplitFiles(userNorm) {
  const directCache = EXTERNAL_QA_CACHE.direct instanceof Map ? EXTERNAL_QA_CACHE.direct : new Map();
  EXTERNAL_QA_CACHE.direct = directCache;
  if (directCache.has(userNorm)) return directCache.get(userNorm);

  const files = orderExternalQaFiles(resolveExternalQaFiles(), userNorm);
  if (!files.length) {
    rememberExternalQaDirect(userNorm, null);
    return null;
  }

  const keyJson = JSON.stringify(userNorm);
  for (const file of files) {
    try {
      const stat = fs.statSync(file.abs);
      if (!stat.isFile()) continue;
      if (stat.size > EXTERNAL_QA_MAX_BYTES) continue;

      const raw = fs.readFileSync(file.abs, 'utf8').replace(/^\uFEFF/, '');
      const value = findJsonObjectValueByExactKey(raw, keyJson);
      if (value === undefined) continue;

      const answer = externalQaValueToAnswer(value);
      if (!answer) continue;

      const hit = {
        answer: answer.slice(0, 6000),
        question: userNorm.slice(0, 300),
        confidence: 1,
        matchType: 'split-exact-json-key',
        source: file.publicPath
      };
      rememberExternalQaDirect(userNorm, hit);
      return hit;
    } catch (_) {
      continue;
    }
...
function orderExternalQaFiles(files, userNorm) {
  const priority = externalQaPriorityFileNumbers(userNorm);
  if (!priority.length) return files;

  const byNumber = new Map();
  for (const file of files) {
    const num = externalQaFileNumber(file.publicPath);
    if (num) byNumber.set(num, file);
  }
  ...
  for (const num of priority) {
    ...
    if (file && !used.has(file.publicPath)) {
      ordered.push(file);
      used.add(file.publicPath);
    }
  }

  for (const file of files) {
    if (!used.has(file.publicPath)) ordered.push(file);
  }
  return ordered;
}

function externalQaFileNumber(publicPath) {
  return Number((String(publicPath || '').match(/qa-(\d+)\.json$/) || [])[1] || 0);
}

function externalQaPriorityFileNumbers(userNorm) {
  const n = normalize(userNorm);
  const out = [];
  const add = (num) => { if (num && !out.includes(num)) out.push(num); };

  const square = n.match(/\bkuadrat\s+dari\s+(\d+)\b/);
  if (square) {
    const value = Number(square[1]);
    if (value <= 137724) add(1);
    else if (value <= 675871) add(2);
    else add(3);
  }
  if (/\bpangkat\s+tiga\b/.test(n)) add(4);
  if (/\bpangkat\s+empat\b/.test(n)) add(5);
  if (/\bbilangan\s+prima\b/.test(n)) add(6);
  if (/\bakar\s+kuadrat\b/.test(n)) add(7);
  if (/\bmod\s+7\b/.test(n)) {
    const modValue = Number((n.match(/\b(\d+)\s+mod\s+7\b/) || [])[1] || 0);
    if (modValue > 737847) add(11);
    add(8);
  }
  if (/\bjumlah\s+dari\b/.test(n)) add(9);
  if (/\bpangkat\s+lima\b/.test(n)) add(10);
  ...
  for (const [fileNo, terms] of socialMap) {
    if (terms.some((term) => n.includes(term))) add(fileNo);
  }
  return out;
}

function rememberExternalQaDirect(key, value) {
  if (!(EXTERNAL_QA_CACHE.direct instanceof Map)) EXTERNAL_QA_CACHE.direct = new Map();
  if (EXTERNAL_QA_CACHE.direct.size > 300) EXTERNAL_QA_CACHE.direct.clear();
  EXTERNAL_QA_CACHE.direct.set(key, value);
}

function externalQaValueToAnswer(value) {
  if (value === null || value === undefined) return '';
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') return String(value).trim();
  if (Array.isArray(value)) return value.map((item) => String(item || '').trim()).filter(Boolean).join('\n').trim();
  if (value && typeof value === 'object') {
    const answer = value.jawaban ?? value.answer ?? value.a ?? value.respon ?? value.response ?? value.value;
...
function findJsonObjectValueByExactKey(raw, keyJson) {
  // Fast path untuk JSON object compact: {"pertanyaan":"jawaban"}
  let pos = raw.indexOf(keyJson);
  while (pos !== -1) {
    let cursor = pos + keyJson.length;
    while (cursor < raw.length && /\s/.test(raw[cursor])) cursor += 1;
    if (raw[cursor] === ':') {
      const parsed = readJsonValueAt(raw, cursor + 1);
      if (parsed && parsed.ok) return parsed.value;
    }
    pos = raw.indexOf(keyJson, pos + keyJson.length);
  }
  return undefined;
}

function readJsonValueAt(raw, index) {
  let start = index;
  ...
  if (first === '"') {
    ...
    while (end < raw.length && raw[end] !== ',' && raw[end] !== '}' && raw[end] !== ']') end += 1;
  }

  try {
    return { ok: true, value: JSON.parse(raw.slice(start, end)) };
  } catch (_) {
    return null;
  }
}
...
function resolveExternalQaFiles() {
  ...
  for (let i = 1; i <= EXTERNAL_QA_SPLIT_COUNT; i += 1) {
    ...
    for (const root of roots) {
      ...
      try {
        if (!seenPublic.has(publicPath) && fs.existsSync(abs) && fs.statSync(abs).isFile()) {
          files.push({ abs, publicPath });
  ...
  return files.sort((a, b) => {
    const ax = Number((a.publicPath.match(/qa-(\d+)\.json$/) || [])[1] || 0);
    const bx = Number((b.publicPath.match(/qa-(\d+)\.json$/) || [])[1] || 0);
    return ax - bx;
  });
}
...
function loadExternalQaEntries() {
  ...
  if (!found) {
    EXTERNAL_QA_CACHE = { signature: 'missing', entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: 'File /data/qa.json belum ditemukan.', loadedAt: Date.now(), source: '/data/qa.json' };
  ...
  try {
    ...
    EXTERNAL_QA_CACHE = { signature: 'stat-error', entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: 'File /data/qa.json belum bisa dibaca.', loadedAt: Date.now(), source: found };
    return EXTERNAL_QA_CACHE;
  }
  ...
  try {
    if (stat.size > EXTERNAL_QA_MAX_BYTES) {
      EXTERNAL_QA_CACHE = { signature, entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: `File /data/qa.json terlalu besar (${stat.size} byte).`, loadedAt: Date.now(), source: found };
      return EXTERNAL_QA_CACHE;
    }
    ...
    EXTERNAL_QA_CACHE = { signature, entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: `File /data/qa.json tidak valid: ${sanitizeError(error)}`, loadedAt: Date.now(), source: found };
    return EXTERNAL_QA_CACHE;
  }
}

...
const STORE = globalThis.__DIRAC_AI_STORE__ || (globalThis.__DIRAC_AI_STORE__ = { rate: new Map(), fingerprint: new Map() });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (intent.name === 'general') {
      const localKnowledge = localGeneralAnswer(normalizedMessage);
      if (localKnowledge && (localKnowledge.forceLocal || !hasProvider())) {
        return res.status(200).json(makeReply('conversation', localKnowledge.reply, {
          traceId,
          intent: intent.name,
          provider: 'local-general-knowledge-router',
          confidence: localKnowledge.confidence || 0.78,
          analytics: { intent: intent.name, source: 'local-general-knowledge-router', ms: Date.now() - startedAt }
        }));
      }
    }
    ...
    if (intent.name === 'compare_products') {
      ...
      return res.status(200).json(makeReply('commerce', comparison.reply, {
        ...
        products: publicProducts(comparison.products, { ...context, requestedCount: extractRequestedCount(normalizedMessage) || 4 }),
    ...
    const requestedCount = extractRequestedCount(normalizedMessage);
    const productLimit = useProducts ? Math.min(10, Math.max(3, requestedCount || 3)) : 0;
    const productContext = { ...context, requestedCount: productLimit };
    const scoredProducts = useProducts ? scoreProducts(products, context, normalizedMessage).slice(0, Math.max(10, productLimit)) : [];
    const topProducts = scoredProducts.slice(0, productLimit).map((item) => item.product);
    ...
    if (useProducts && topProducts.length && !hasProvider()) {
      return res.status(200).json(makeReply('commerce', buildProductReply(topProducts, context), {
        ...
        provider: 'local-secure-product-matcher',
        showProducts: true,
        products: publicProducts(topProducts, productContext),
    ...
    if (!hasProvider()) {
      const localKnowledge = intent.name === 'general' ? localGeneralAnswer(normalizedMessage) : null;
      const fallbackText = localKnowledge
        ? localKnowledge.reply
        : 'AI utama belum aktif karena API key belum disetel di Vercel. Saya masih bisa bantu link website, cek resi, cara checkout, harga katalog, dan rekomendasi dasar. Untuk pertanyaan umum yang sangat luas/terbaru, aktifkan API AI utama agar jawabannya lebih lengkap.';
      return res.status(200).json(makeReply(localKnowledge ? 'conversation' : 'fallback', fallbackText, {
        traceId,
        intent: intent.name,
        provider: localKnowledge ? 'local-general-knowledge-router' : 'local-fallback',
        confidence: localKnowledge ? (localKnowledge.confidence || 0.72) : 0.55
      }));
    }
    ...
    try {
      ...
      logAi('provider_fallback', { traceId, intent: intent.name, message: sanitizeError(providerError) });
      const localKnowledge = intent.name === 'general' ? localGeneralAnswer(normalizedMessage) : null;
      if (localKnowledge) {
        return res.status(200).json(makeReply('conversation', localKnowledge.reply, {
          traceId,
          intent: intent.name,
          provider: 'local-general-fallback-after-provider-error',
          confidence: localKnowledge.confidence || 0.64,
          analytics: { intent: intent.name, source: 'local-general-fallback-after-provider-error', ms: Date.now() - startedAt }
        }));
      }
      if (useProducts && topProducts.length) {
        return res.status(200).json(makeReply('commerce', buildProductReply(topProducts, context), {
          traceId,
          provider: 'local-secure-product-matcher-after-provider-error',
          showProducts: true,
          products: publicProducts(topProducts, productContext),
          budgetMatched: budgetMatched(topProducts, context),
          intent: intent.name,
          confidence: 0.80,
          recommendationContext: publicContext(context),
          analytics: { intent: intent.name, source: 'local-secure-product-matcher-after-provider-error', ms: Date.now() - startedAt }
        }));
      }
      throw providerError;
    }
    ...
    return res.status(200).json(makeReply(useProducts ? 'commerce' : intent.mode, safeText, {
      ...
      products: useProducts ? publicProducts(topProducts, productContext) : [],
...
function isGeneralKnowledge(text) {
  const productTerms = /\b(parfum|perfume|produk parfum|wangi parfum|aroma parfum|botol parfum|ml parfum|stok|ready|rekomendasi parfum|checkout|keranjang|resi|paket|kurir|katalog|website ini|produk dirac|dirac|lv|louis vuitton|xerjoff|ysl|yves saint laurent|jpg|jean paul gaultier|miniso|mykonos)\b/.test(text);
...
function extractRequestedCount(text) {
  ...
  const words = { satu:1, dua:2, tiga:3, empat:4, lima:5, enam:6, tujuh:7, delapan:8, sembilan:9, sepuluh:10 };
  const digit = n.match(/\b(?:sebutkan|tampilkan|kasih|beri|pilihkan|rekomendasikan|list|daftar|minta|mau|ingin)?\s*(\d{1,2})\s*(?:produk|parfum|item|pilihan|rekomendasi)\b/);
  if (digit) return Math.min(10, Math.max(1, Number(digit[1]) || 0));
  for (const [word, value] of Object.entries(words)) {
    if (new RegExp('\\b' + word + '\\s+(?:produk|parfum|item|pilihan|rekomendasi)\\b').test(n)) return value;
  }
  return 0;
}
...
function localGeneralAnswer(text) {
  const n = normalize(text);
  const math = localMathAnswer(n);
  if (math) return { reply: math, confidence: 0.97, forceLocal: true, domain: 'math' };
  const realtime = localRealtimeBoundary(n);
  if (realtime) return { reply: realtime, confidence: 0.90, forceLocal: true, domain: 'realtime-boundary' };
  const geography = localGeographyAnswer(n);
  if (geography) return { reply: geography, confidence: 0.92, forceLocal: true, domain: 'geography' };
  const world = localWorldAnswer(n);
  if (world) return { reply: world, confidence: 0.90, forceLocal: true, domain: 'world-knowledge' };
  const generalLike = isGeneralKnowledge(n);
  const concept = generalLike ? localConceptAnswer(n) : '';
  if (concept) return { reply: concept, confidence: 0.82, forceLocal: false, domain: 'concept' };
  if (generalLike) {
    return { reply: buildUniversalGeneralReply(n), confidence: 0.66, forceLocal: false, domain: 'universal-general' };
  }
  return null;
}


function localMathAnswer(n) {
  const original = n;
  const discount = n.match(/(?:diskon|discount)\s*(\d+(?:[.,]\d+)?)\s*(?:%|persen)?\s*(?:dari|untuk)?\s*(?:rp\s*)?([\d., ]+)/);
  if (discount) {
    const pct = Number(discount[1].replace(',', '.'));
    const base = parseLooseNumber(discount[2].replace(/\s+/g, ''));
    if (base > 0) {
      const cut = Math.round(base * pct / 100);
      return `Diskon ${pct}% dari ${formatIDRLocal(base)} adalah ${formatIDRLocal(cut)}. Total setelah diskon: ${formatIDRLocal(base - cut)}.`;
    }
  }
  const percent = n.match(/(\d+(?:[.,]\d+)?)\s*(?:%|persen)\s*(?:dari|x|\*)\s*(?:rp\s*)?([\d., ]+)/);
  if (percent) {
    const pct = Number(percent[1].replace(',', '.'));
    const base = parseLooseNumber(percent[2]);
    if (base > 0) return `${pct}% dari ${formatNumberLocal(base)} = ${formatNumberLocal(base * pct / 100)}.`;
  }
  if (/\b(akar|sqrt)\s*(\d+(?:[.,]\d+)?)\b/.test(n)) {
    const v = Number(n.match(/\b(?:akar|sqrt)\s*(\d+(?:[.,]\d+)?)\b/)[1].replace(',', '.'));
    return `Akar dari ${formatNumberLocal(v)} = ${formatNumberLocal(Math.sqrt(v))}.`;
  }
  const power = n.match(/(\d+(?:[.,]\d+)?)\s*(?:pangkat|\^)\s*(\d+(?:[.,]\d+)?)/);
  if (power) {
    const a = Number(power[1].replace(',', '.'));
    const b = Number(power[2].replace(',', '.'));
    return `${formatNumberLocal(a)} pangkat ${formatNumberLocal(b)} = ${formatNumberLocal(Math.pow(a,b))}.`;
  }
  const exprMatch = original.match(/(?:^|\b)([\d\s.,+\-*/x:()]+)\s*(?:=|berapa|hasilnya|hitung)?\s*$/);
  if (exprMatch && /\d/.test(exprMatch[1]) && /[+\-*/x:]/.test(exprMatch[1])) {
    let expr = exprMatch[1].trim();
    expr = expr.replace(/(\d),(\d)/g, '$1.$2').replace(/(?<=\d)\.(?=\d{3}(\D|$))/g, '');
    expr = expr.replace(/x/g, '*').replace(/:/g, '/');
    if (/^[\d\s.+\-*/()]+$/.test(expr) && expr.length <= 220) {
      try {
        const value = Function('"use strict"; return (' + expr + ');')();
        if (Number.isFinite(value)) return `${exprMatch[1].trim()} = ${formatNumberLocal(value)}.`;
      } catch (error) {}
    }
  }
  return '';
...
function formatNumberLocal(value) {
  ...
  const rounded = Math.abs(num % 1) < 1e-10 ? Math.round(num) : Number(num.toFixed(6));
  return new Intl.NumberFormat('id-ID', { maximumFractionDigits: 6 }).format(rounded);
}
function formatIDRLocal(value) { return 'Rp' + new Intl.NumberFormat('id-ID', { maximumFractionDigits: 0 }).format(Math.round(Number(value) || 0)); }

function localRealtimeBoundary(n) {
  if (/\b(hari ini|sekarang|terbaru|real time|realtime|live)\b/.test(n) && /\b(harga|kurs|cuaca|saham|emas|crypto|bitcoin|btc|dolar|usd|mobil|motor|berita)\b/.test(n)) {
    return 'Topik itu membutuhkan data real-time. Saya tidak akan mengarang angka terbaru. Cek sumber resmi/live market untuk angka hari ini. Untuk produk Dirac, saya bisa membaca harga dari katalog jika Anda menyebutkan nama produknya.';
  }
  return '';
}


function localGeographyAnswer(n) {
  const admin = {
    aceh: { kab:18, kota:5, kec:'sekitar 290 kecamatan', note:'Aceh memiliki 18 kabupaten dan 5 kota. Jumlah kecamatan umumnya sekitar 290 dan dapat berubah mengikuti penataan wilayah.' },
    'sumatera utara': { kab:25, kota:8, kec:'455 kecamatan', note:'Sumatera Utara memiliki 25 kabupaten dan 8 kota.' },
    'sumatera barat': { kab:12, kota:7, kec:'179 kecamatan', note:'Sumatera Barat memiliki 12 kabupaten dan 7 kota.' },
    riau: { kab:10, kota:2, kec:'169 kecamatan', note:'Riau memiliki 10 kabupaten dan 2 kota.' },
    'kepulauan riau': { kab:5, kota:2, kec:'80 kecamatan', note:'Kepulauan Riau memiliki 5 kabupaten dan 2 kota.' },
    jambi: { kab:9, kota:2, kec:'144 kecamatan', note:'Jambi memiliki 9 kabupaten dan 2 kota.' },
    'sumatera selatan': { kab:13, kota:4, kec:'241 kecamatan', note:'Sumatera Selatan memiliki 13 kabupaten dan 4 kota.' },
    bengkulu: { kab:9, kota:1, kec:'129 kecamatan', note:'Bengkulu memiliki 9 kabupaten dan 1 kota.' },
    lampung: { kab:13, kota:2, kec:'228 kecamatan', note:'Lampung memiliki 13 kabupaten dan 2 kota.' },
    'bangka belitung': { kab:6, kota:1, kec:'47 kecamatan', note:'Kepulauan Bangka Belitung memiliki 6 kabupaten dan 1 kota.' },
    jakarta: { kab:1, kota:5, kec:'44 kecamatan', note:'DKI Jakarta memiliki 1 kabupaten administratif, yaitu Kepulauan Seribu, dan 5 kota administrasi. Total kecamatannya 44.' },
    'jawa barat': { kab:18, kota:9, kec:'627 kecamatan', note:'Jawa Barat memiliki 18 kabupaten dan 9 kota.' },
    'jawa tengah': { kab:29, kota:6, kec:'576 kecamatan', note:'Jawa Tengah memiliki 29 kabupaten dan 6 kota.' },
    yogyakarta: { kab:4, kota:1, kec:'78 kapanewon/kemantren', note:'DI Yogyakarta memiliki 4 kabupaten dan 1 kota.' },
    'jawa timur': { kab:29, kota:9, kec:'666 kecamatan', note:'Jawa Timur memiliki 29 kabupaten dan 9 kota.' },
    banten: { kab:4, kota:4, kec:'155 kecamatan', note:'Banten memiliki 4 kabupaten dan 4 kota.' },
    bali: { kab:8, kota:1, kec:'57 kecamatan', note:'Bali memiliki 8 kabupaten dan 1 kota.' },
    'nusa tenggara barat': { kab:8, kota:2, kec:'117 kecamatan', note:'Nusa Tenggara Barat memiliki 8 kabupaten dan 2 kota.' },
    'nusa tenggara timur': { kab:21, kota:1, kec:'309 kecamatan', note:'Nusa Tenggara Timur memiliki 21 kabupaten dan 1 kota.' },
    'kalimantan barat': { kab:12, kota:2, kec:'174 kecamatan', note:'Kalimantan Barat memiliki 12 kabupaten dan 2 kota.' },
    'kalimantan tengah': { kab:13, kota:1, kec:'136 kecamatan', note:'Kalimantan Tengah memiliki 13 kabupaten dan 1 kota.' },
    'kalimantan selatan': { kab:11, kota:2, kec:'156 kecamatan', note:'Kalimantan Selatan memiliki 11 kabupaten dan 2 kota.' },
    'kalimantan timur': { kab:7, kota:3, kec:'105 kecamatan', note:'Kalimantan Timur memiliki 7 kabupaten dan 3 kota.' },
    'kalimantan utara': { kab:4, kota:1, kec:'55 kecamatan', note:'Kalimantan Utara memiliki 4 kabupaten dan 1 kota.' },
    'sulawesi utara': { kab:11, kota:4, kec:'171 kecamatan', note:'Sulawesi Utara memiliki 11 kabupaten dan 4 kota.' },
    'sulawesi tengah': { kab:12, kota:1, kec:'175 kecamatan', note:'Sulawesi Tengah memiliki 12 kabupaten dan 1 kota.' },
    'sulawesi selatan': { kab:21, kota:3, kec:'313 kecamatan', note:'Sulawesi Selatan memiliki 21 kabupaten dan 3 kota.' },
    'sulawesi tenggara': { kab:15, kota:2, kec:'221 kecamatan', note:'Sulawesi Tenggara memiliki 15 kabupaten dan 2 kota.' },
    gorontalo: { kab:5, kota:1, kec:'77 kecamatan', note:'Gorontalo memiliki 5 kabupaten dan 1 kota.' },
    'sulawesi barat': { kab:6, kota:0, kec:'69 kecamatan', note:'Sulawesi Barat memiliki 6 kabupaten dan belum memiliki kota administratif.' },
    maluku: { kab:9, kota:2, kec:'118 kecamatan', note:'Maluku memiliki 9 kabupaten dan 2 kota.' },
    'maluku utara': { kab:8, kota:2, kec:'118 kecamatan', note:'Maluku Utara memiliki 8 kabupaten dan 2 kota.' },
    papua: { kab:8, kota:1, kec:'105 distrik', note:'Papua memiliki 8 kabupaten dan 1 kota setelah pemekaran provinsi baru.' },
    'papua barat': { kab:7, kota:0, kec:'86 distrik', note:'Papua Barat memiliki 7 kabupaten.' },
    'papua tengah': { kab:8, kota:0, kec:'131 distrik', note:'Papua Tengah memiliki 8 kabupaten.' },
    'papua pegunungan': { kab:8, kota:0, kec:'252 distrik', note:'Papua Pegunungan memiliki 8 kabupaten.' },
    'papua selatan': { kab:4, kota:0, kec:'82 distrik', note:'Papua Selatan memiliki 4 kabupaten.' },
    'papua barat daya': { kab:5, kota:1, kec:'132 distrik', note:'Papua Barat Daya memiliki 5 kabupaten dan 1 kota.' }
  };
  const island = {
    jawa: { kab:85, kota:34, kec:'sekitar 2.146 kecamatan/kapanewon/kemantren', note:'Pulau Jawa mencakup Banten, DKI Jakarta, Jawa Barat, Jawa Tengah, DI Yogyakarta, dan Jawa Timur. Totalnya sekitar 85 kabupaten dan 34 kota.' },
    kalimantan: { kab:47, kota:9, kec:'sekitar 626 kecamatan', note:'Kalimantan Indonesia mencakup Kalimantan Barat, Tengah, Selatan, Timur, dan Utara. Totalnya sekitar 47 kabupaten dan 9 kota.' },
    sulawesi: { kab:70, kota:11, kec:'sekitar 1.026 kecamatan', note:'Sulawesi mencakup Sulawesi Utara, Tengah, Selatan, Tenggara, Barat, dan Gorontalo. Totalnya sekitar 70 kabupaten dan 11 kota.' },
    sumatera: { kab:120, kota:34, kec:'sekitar 1.900+ kecamatan', note:'Sumatra mencakup Aceh, Sumatera Utara, Barat, Riau, Kepulauan Riau, Jambi, Sumatera Selatan, Bengkulu, Lampung, dan Bangka Belitung. Totalnya sekitar 120 kabupaten dan 34 kota.' },
    papua: { kab:40, kota:2, kec:'sekitar 700+ distrik', note:'Wilayah Papua Indonesia kini terbagi ke beberapa provinsi: Papua, Papua Barat, Papua Tengah, Papua Pegunungan, Papua Selatan, dan Papua Barat Daya.' }
  };
  if (/\b(ada berapa|berapa|jumlah)\b/.test(n) && /\b(provinsi|propinsi)\b/.test(n) && /\bindonesia\b/.test(n)) return 'Indonesia memiliki 38 provinsi.';
  if (/\b(ada berapa|berapa|jumlah)\b/.test(n) && /\b(kabupaten|kota|kecamatan|distrik)\b/.test(n) && /\bindonesia\b/.test(n)) return 'Secara umum Indonesia memiliki 38 provinsi, sekitar 416 kabupaten, 98 kota, dan lebih dari 7.000 kecamatan/distrik/kapanewon/kemantren. Angka rinci bisa berubah mengikuti pemekaran dan pembaruan administrasi.';
  for (const [key, value] of Object.entries(admin)) {
    if (n.includes(key)) {
      if (/\bkecamatan|distrik\b/.test(n)) return `${capitalizeLocal(key)} memiliki ${value.kec}. ${value.note}`;
      if (/\bkota\b/.test(n)) return `${capitalizeLocal(key)} memiliki ${value.kota} kota. ${value.note}`;
      if (/\bkabupaten\b/.test(n)) return `${capitalizeLocal(key)} memiliki ${value.kab} kabupaten. ${value.note}`;
      if (/\bprovinsi|dimana|di mana|ibukota|ibu kota\b/.test(n)) return `${capitalizeLocal(key)} adalah wilayah/provinsi di Indonesia. ${value.note}`;
    }
  }
  for (const [key, value] of Object.entries(island)) {
    if (n.includes(key) && /\b(kabupaten|kota|kecamatan|distrik)\b/.test(n)) {
      if (/\bkecamatan|distrik\b/.test(n)) return `${value.note} Jumlah kecamatan/distriknya ${value.kec}, tergantung pembaruan administrasi.`;
      return value.note;
    }
  }
  if (/\b(ibu kota|ibukota)\b/.test(n) && /\bindonesia\b/.test(n)) return 'Ibu kota negara Indonesia saat ini masih Jakarta secara administratif, sementara Ibu Kota Nusantara (IKN) sedang dikembangkan sebagai ibu kota baru sesuai tahapan pemindahan pemerintahan.';
  return '';
}


function capitalizeLocal(text) { return String(text).split(' ').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' '); }


function localWorldAnswer(n) {
  if (/\b(semua negara|daftar negara|negara di dunia|berapa negara)\b/.test(n)) {
    const countries = 'Afganistan, Afrika Selatan, Albania, Aljazair, Amerika Serikat, Andorra, Angola, Antigua dan Barbuda, Arab Saudi, Argentina, Armenia, Australia, Austria, Azerbaijan, Bahama, Bahrain, Bangladesh, Barbados, Belanda, Belarus, Belgia, Belize, Benin, Bhutan, Bolivia, Bosnia dan Herzegovina, Botswana, Brasil, Brunei, Bulgaria, Burkina Faso, Burundi, Cabo Verde, Ceko, Chad, Chili, China, Denmark, Djibouti, Dominika, Ekuador, El Salvador, Eritrea, Estonia, Eswatini, Ethiopia, Fiji, Filipina, Finlandia, Gabon, Gambia, Georgia, Ghana, Grenada, Guatemala, Guinea, Guinea-Bissau, Guinea Khatulistiwa, Guyana, Haiti, Honduras, Hungaria, India, Indonesia, Inggris/Raya Britania, Irak, Iran, Irlandia, Islandia, Israel, Italia, Jamaika, Jepang, Jerman, Kamboja, Kamerun, Kanada, Kazakhstan, Kenya, Kirgizstan, Kiribati, Kolombia, Komoro, Kongo, Korea Selatan, Korea Utara, Kosta Rika, Kroasia, Kuba, Kuwait, Laos, Latvia, Lebanon, Lesotho, Liberia, Libya, Liechtenstein, Lituania, Luksemburg, Madagaskar, Malawi, Malaysia, Maladewa, Mali, Malta, Maroko, Marshall, Mauritania, Mauritius, Meksiko, Mesir, Mikronesia, Moldova, Monako, Mongolia, Montenegro, Mozambik, Myanmar, Namibia, Nauru, Nepal, Niger, Nigeria, Nikaragua, Norwegia, Oman, Pakistan, Palau, Palestina, Panama, Pantai Gading, Papua Nugini, Paraguay, Peru, Polandia, Portugal, Prancis, Qatar, Republik Afrika Tengah, Republik Demokratik Kongo, Republik Dominika, Rumania, Rusia, Rwanda, Saint Kitts dan Nevis, Saint Lucia, Saint Vincent dan Grenadine, Samoa, San Marino, Sao Tome dan Principe, Selandia Baru, Senegal, Serbia, Seychelles, Sierra Leone, Singapura, Siprus, Slovenia, Slovakia, Somalia, Spanyol, Sri Lanka, Sudan, Sudan Selatan, Suriah, Suriname, Swedia, Swiss, Tajikistan, Tanzania, Thailand, Timor-Leste, Togo, Tonga, Trinidad dan Tobago, Tunisia, Turki, Turkmenistan, Tuvalu, Uganda, Ukraina, Uni Emirat Arab, Uruguay, Uzbekistan, Vanuatu, Vatikan, Venezuela, Vietnam, Yaman, Yordania, Yunani, Zambia, Zimbabwe.';
    return `Ada 193 negara anggota PBB. Jika memasukkan negara pengamat/diakui luas seperti Vatikan dan Palestina, jumlah yang sering dipakai adalah 195. Daftar ringkas: ${countries}`;
  }
  if (/\b(benua)\b/.test(n) && /\b(berapa|sebutkan|daftar|nama)\b/.test(n)) return 'Umumnya ada 7 benua: Asia, Afrika, Amerika Utara, Amerika Selatan, Antarktika, Eropa, dan Australia/Oseania.';
  if (/\b(samudra|lautan)\b/.test(n) && /\b(berapa|sebutkan|daftar|nama)\b/.test(n)) return 'Umumnya ada 5 samudra: Pasifik, Atlantik, Hindia, Selatan/Antarktika, dan Arktik.';
  if (/\b(planet)\b/.test(n) && /\b(berapa|sebutkan|daftar|nama)\b/.test(n)) return 'Tata Surya memiliki 8 planet: Merkurius, Venus, Bumi, Mars, Jupiter, Saturnus, Uranus, dan Neptunus.';
  if (/\b(presiden arab saudi|semua presiden arab saudi)\b/.test(n)) return 'Arab Saudi tidak memiliki presiden karena bentuk negaranya monarki. Kepala negaranya adalah raja. Raja-raja Arab Saudi modern: Abdulaziz, Saud, Faisal, Khalid, Fahd, Abdullah, dan Salman.';
  if (/\b(raja arab saudi|semua raja arab saudi)\b/.test(n)) return 'Raja-raja Arab Saudi modern: Abdulaziz bin Saud, Saud bin Abdulaziz, Faisal bin Abdulaziz, Khalid bin Abdulaziz, Fahd bin Abdulaziz, Abdullah bin Abdulaziz, dan Salman bin Abdulaziz.';
  if (/\b(asean)\b/.test(n)) return 'ASEAN adalah organisasi negara-negara Asia Tenggara untuk kerja sama ekonomi, politik, keamanan, pendidikan, dan sosial budaya. Anggotanya: Indonesia, Malaysia, Singapura, Thailand, Filipina, Brunei, Vietnam, Laos, Myanmar, Kamboja, dan Timor-Leste dalam proses keanggotaan penuh.';
  if (/\b(pbb|perserikatan bangsa bangsa|united nations)\b/.test(n)) return 'PBB atau Perserikatan Bangsa-Bangsa adalah organisasi internasional yang dibentuk untuk menjaga perdamaian, kerja sama antarnegara, hak asasi manusia, bantuan kemanusiaan, dan pembangunan. Anggota PBB berjumlah 193 negara.';
  if (/\b(ibukota|ibu kota)\b/.test(n) && /\b(jepang|japan)\b/.test(n)) return 'Ibu kota Jepang adalah Tokyo.';
  if (/\b(ibukota|ibu kota)\b/.test(n) && /\b(amerika serikat|as|usa|united states)\b/.test(n)) return 'Ibu kota Amerika Serikat adalah Washington, D.C.';
  if (/\b(ibukota|ibu kota)\b/.test(n) && /\b(australia)\b/.test(n)) return 'Ibu kota Australia adalah Canberra.';
  return '';
}



function localConceptAnswer(n) {
  const concepts = [
    ['gravitasi', 'Gravitasi adalah gaya tarik-menarik antara benda bermassa. Di Bumi, gravitasi membuat benda jatuh ke bawah dan menjaga atmosfer serta benda tetap tertarik ke pusat Bumi.'],
    ['atom', 'Atom adalah unit dasar penyusun materi. Atom terdiri dari inti yang berisi proton dan neutron, serta elektron yang bergerak di sekitar inti.'],
    ['molekul', 'Molekul adalah gabungan dua atom atau lebih yang terikat secara kimia, misalnya H2O untuk air dan O2 untuk oksigen.'],
    ['fotosintesis', 'Fotosintesis adalah proses tumbuhan membuat makanan dari air, karbon dioksida, dan cahaya matahari dengan bantuan klorofil, menghasilkan glukosa dan oksigen.'],
    ['sel', 'Sel adalah unit terkecil makhluk hidup. Sel menjalankan fungsi dasar kehidupan seperti metabolisme, pertumbuhan, reproduksi, dan respons terhadap lingkungan.'],
    ['dna', 'DNA adalah molekul pembawa informasi genetik yang mengatur sifat makhluk hidup dan diwariskan dari orang tua ke keturunan.'],
    ['evolusi', 'Evolusi adalah perubahan sifat makhluk hidup dari generasi ke generasi melalui variasi, seleksi alam, mutasi, dan adaptasi.'],
    ['ekosistem', 'Ekosistem adalah hubungan antara makhluk hidup dan lingkungan fisiknya, seperti hutan, laut, sungai, sawah, atau gurun.'],
    ['demokrasi', 'Demokrasi adalah sistem pemerintahan yang memberi rakyat peran dalam pengambilan keputusan, biasanya melalui pemilu, kebebasan berpendapat, dan perwakilan.'],
    ['inflasi', 'Inflasi adalah kenaikan harga barang dan jasa secara umum dalam suatu periode, sehingga daya beli uang menurun.'],
    ['deflasi', 'Deflasi adalah penurunan harga barang dan jasa secara umum. Deflasi bisa terlihat baik, tetapi dapat menandakan permintaan melemah dan ekonomi melambat.'],
    ['resesi', 'Resesi adalah kondisi penurunan aktivitas ekonomi secara luas selama periode tertentu, biasanya ditandai pertumbuhan ekonomi negatif, pengangguran naik, dan konsumsi melemah.'],
    ['saham', 'Saham adalah bukti kepemilikan sebagian perusahaan. Pemilik saham berpotensi mendapat keuntungan dari kenaikan harga saham atau dividen, tetapi juga menanggung risiko kerugian.'],
    ['obligasi', 'Obligasi adalah surat utang. Penerbit obligasi meminjam uang dari investor dan membayar bunga serta pokok sesuai jadwal.'],
    ['html', 'HTML adalah bahasa markup untuk menyusun struktur halaman web, seperti judul, paragraf, gambar, tautan, formulir, dan tombol.'],
    ['css', 'CSS adalah bahasa untuk mengatur tampilan halaman web, seperti warna, ukuran, layout, animasi, jarak, dan responsif di mobile.'],
    ['javascript', 'JavaScript adalah bahasa pemrograman yang membuat website interaktif, misalnya tombol, keranjang, validasi form, animasi, dan pemanggilan API.'],
    ['python', 'Python adalah bahasa pemrograman yang populer untuk automasi, analisis data, AI, backend, scripting, dan pembelajaran pemrograman.'],
    ['api', 'API adalah antarmuka agar dua sistem bisa berkomunikasi. Contohnya website mengirim pertanyaan ke server AI melalui endpoint API.'],
    ['database', 'Database adalah tempat menyimpan dan mengelola data secara terstruktur, misalnya produk, stok, pesanan, pengguna, dan chat log.'],
    ['algoritma', 'Algoritma adalah langkah-langkah logis untuk menyelesaikan masalah, seperti mencari produk paling cocok berdasarkan harga, kategori, dan stok.'],
    ['internet', 'Internet adalah jaringan global yang menghubungkan komputer dan perangkat di seluruh dunia untuk bertukar data.'],
    ['cloud', 'Cloud computing adalah penggunaan server melalui internet untuk menyimpan data, menjalankan aplikasi, dan memproses layanan tanpa harus memiliki server fisik sendiri.'],
    ['ai', 'AI atau kecerdasan buatan adalah teknologi yang membuat komputer dapat melakukan tugas yang biasanya memerlukan kecerdasan manusia, seperti memahami bahasa, mengenali pola, dan memberi rekomendasi.'],
    ['machine learning', 'Machine learning adalah cabang AI yang membuat sistem belajar dari data untuk membuat prediksi atau keputusan tanpa diprogram aturan satu per satu.'],
    ['deep learning', 'Deep learning adalah cabang machine learning yang memakai jaringan saraf berlapis untuk mempelajari pola kompleks, misalnya gambar, suara, dan bahasa.'],
    ['marketing', 'Marketing adalah proses memahami kebutuhan pasar, membangun nilai produk, menentukan target audiens, menyampaikan pesan, dan mendorong penjualan.'],
    ['branding', 'Branding adalah proses membangun identitas dan persepsi merek, termasuk nama, logo, warna, suara komunikasi, kualitas, dan pengalaman pelanggan.'],
    ['manajemen', 'Manajemen adalah proses merencanakan, mengorganisasi, mengarahkan, dan mengendalikan sumber daya agar tujuan tercapai.'],
    ['akuntansi', 'Akuntansi adalah proses mencatat, mengelompokkan, meringkas, dan melaporkan transaksi keuangan agar kondisi bisnis dapat dipahami.'],
    ['hukum', 'Hukum adalah aturan yang mengatur perilaku masyarakat dan negara. Untuk kasus hukum nyata, sebaiknya konsultasi dengan ahli hukum.'],
    ['kesehatan', 'Kesehatan adalah kondisi fisik, mental, dan sosial yang baik. Untuk gejala medis nyata, sebaiknya konsultasi dengan tenaga kesehatan.'],
    ['parfum', 'Parfum adalah campuran bahan aromatik, pelarut, dan fiksatif yang memberi aroma. Kategori umum meliputi fresh, citrus, woody, floral, spicy, sweet, amber, musk, dan oud.']
  ];
  for (const [key, value] of concepts) if (n.includes(key)) return value;

  const domains = [
    ['matematika','Matematika membahas bilangan, pola, bentuk, perubahan, dan logika. Materinya meliputi aritmetika, aljabar, geometri, trigonometri, statistika, peluang, kalkulus, persamaan, fungsi, matriks, limit, turunan, integral, dan soal cerita.'],
    ['fisika','Fisika mempelajari materi, energi, gerak, gaya, gelombang, listrik, magnet, panas, optik, fluida, atom, relativitas, kuantum, dan alam semesta.'],
    ['kimia','Kimia mempelajari zat, atom, molekul, ikatan, reaksi, larutan, asam-basa, stoikiometri, termokimia, elektrokimia, kimia organik, anorganik, dan material.'],
    ['biologi','Biologi mempelajari makhluk hidup, sel, genetika, evolusi, ekosistem, organ tubuh, tumbuhan, hewan, mikroorganisme, metabolisme, reproduksi, dan kesehatan.'],
    ['sejarah','Sejarah mempelajari peristiwa masa lalu, tokoh, kerajaan, kolonialisme, perang, kemerdekaan, revolusi, peradaban, dan perubahan sosial dari waktu ke waktu.'],
    ['geografi','Geografi mempelajari lokasi, wilayah, peta, iklim, penduduk, sumber daya, bencana, negara, kota, kabupaten, interaksi manusia-lingkungan, dan keruangan.'],
    ['ekonomi','Ekonomi mempelajari produksi, distribusi, konsumsi, pasar, harga, inflasi, uang, bank, investasi, kebijakan, bisnis, perdagangan, dan kesejahteraan.'],
    ['sosiologi','Sosiologi mempelajari masyarakat, interaksi sosial, kelompok, norma, budaya, konflik, perubahan sosial, lembaga sosial, penyimpangan, dan stratifikasi.'],
    ['politik','Politik mempelajari kekuasaan, negara, pemerintahan, partai, kebijakan publik, demokrasi, pemilu, ideologi, diplomasi, dan hubungan internasional.'],
    ['bahasa','Bahasa membahas kosakata, tata bahasa, makna, komunikasi, menulis, membaca, berbicara, terjemahan, pragmatik, semantik, dan gaya penyampaian.'],
    ['coding','Coding adalah proses menulis instruksi untuk komputer. Bidangnya meliputi HTML, CSS, JavaScript, Python, database, API, algoritma, debugging, keamanan, dan deployment.'],
    ['agama','Agama membahas kepercayaan, ibadah, akhlak, sejarah keagamaan, nilai moral, dan hubungan manusia dengan Tuhan, sesama, dan lingkungan. Jawaban sensitif sebaiknya tetap menghormati perbedaan.'],
    ['seni','Seni membahas ekspresi kreatif seperti musik, tari, rupa, desain, teater, fotografi, film, sastra, dan estetika.'],
    ['olahraga','Olahraga membahas aktivitas fisik, teknik latihan, strategi permainan, kebugaran, pemulihan, nutrisi, dan sportivitas.'],
    ['psikologi','Psikologi mempelajari pikiran, emosi, perilaku, motivasi, kepribadian, pembelajaran, perkembangan, kesehatan mental, dan hubungan sosial.'],
    ['filsafat','Filsafat mempelajari pertanyaan mendasar tentang realitas, pengetahuan, moral, logika, bahasa, manusia, dan makna hidup.']
  ];
  for (const [key, value] of domains) if (n.includes(key)) return value + ' Sebutkan subtopik atau soal spesifiknya, nanti saya jabarkan dengan contoh dan latihan.';
  return '';
}



function buildUniversalGeneralReply(n) {
  if (/\b(10000|10\.000|sepuluh ribu)\b/.test(n) && /\b(materi|pertanyaan|soal|bank soal|semua bidang|setiap bidang)\b/.test(n)) {
    return 'Mode bank materi luas aktif. Untuk menjaga website tetap ringan, saya tidak menaruh 10.000 materi statis per bidang di HTML. Saya bisa membuat materi/pertanyaan bertahap per bidang sesuai permintaan. Format yang tersedia: Matematika, Fisika, Kimia, Biologi, Geografi, Sejarah, Ekonomi, Sosiologi, Politik, Bahasa, Coding, Agama, Seni, Olahraga, Psikologi, Filsafat, Bisnis, dan Teknologi. Contoh perintah: "buat 100 soal matematika aljabar", "buat 50 materi biologi sel", atau "buat roadmap 10.000 pertanyaan geografi bertahap".';
  }
  if (/\b(contoh kode html|tombol html|button html)\b/.test(n)) return 'Contoh tombol HTML sederhana:\n```html\n<button type="button">Beli Sekarang</button>\n```\nAgar lebih menarik, tombol bisa diberi CSS untuk warna, radius, dan efek hover.';
  if (/\b(terjemah|translate)\b/.test(n)) return 'Bisa. Kirim kalimat yang ingin diterjemahkan dan sebutkan bahasa tujuannya, misalnya: "terjemahkan ke bahasa Inggris: saya ingin membeli parfum".';
  if (/\b(caption|copywriting|promosi)\b/.test(n)) return 'Bisa. Untuk caption promosi yang kuat, gunakan struktur: hook singkat, manfaat utama, bukti/keunikan, ajakan bertindak. Contoh: "Wangi rapi, tahan lama, dan siap bikin penampilan lebih percaya diri. Pilih parfum favoritmu sekarang dan checkout mudah via WhatsApp."';
  if (/\b(ringkas|rangkum|rangkuman)\b/.test(n)) return 'Bisa. Kirim teks yang ingin diringkas. Saya bisa membuat ringkasan singkat, poin-poin penting, kesimpulan, atau versi bahasa yang lebih mudah dipahami.';
  if (/\b(jelaskan|apa itu|bagaimana|kenapa|mengapa|contoh|materi|soal|tugas|pr)\b/.test(n)) {
    return 'Ini pertanyaan umum. Saya bisa menjawab materi pelajaran, sains, sosial, bahasa, coding, logika, dan pengetahuan umum. Agar jawaban ultra akurat, tulis topiknya lebih spesifik. Jika topiknya membutuhkan data terbaru atau sangat luas, saya akan menjawab dengan batasan yang jelas dan tidak mengarang angka real-time.';
  }
  return 'Saya bisa membantu pertanyaan umum, pelajaran, matematika, sains, sosial, bahasa, coding, pengetahuan dunia, dan katalog Dirac. Tulis topik yang lebih spesifik agar saya bisa menjawab langsung dan tidak mengubahnya menjadi rekomendasi parfum.';
}


function detectIntent(text, history, context, forcedGeneral) {
  if (localMathAnswer(text) || localRealtimeBoundary(text) || localGeographyAnswer(text) || localWorldAnswer(text) || (isGeneralKnowledge(text) && localConceptAnswer(text))) return { name: 'general', mode: 'conversation' };
  ...
  if (/\b(website|web|situs|link|company profile|profil perusahaan|profile perusahaan|alamat web|alamat website)\b/.test(text) && !/\b(parfum|produk|resi|checkout|beli|harga|lv|louis vuitton|xerjoff|ysl|yves saint laurent|jpg|jean paul gaultier|miniso|mykonos|katalog)\b/.test(text)) return { name: 'website', mode: 'link' };
  ...
  const product = /\b(produk|parfum|perfume|wangi|aroma|botol|ml|stok|ready|budget|dana|mahal|murah)\b/.test(text) || (/\b(harga|berapa)\b/.test(text) && /\b(website ini|katalog|dirac|lv|louis vuitton|xerjoff|ysl|yves saint laurent|jpg|jean paul gaultier|miniso|mykonos|parfum)\b/.test(text)) || categoryProduct;
...
function publicProducts(list, context = {}) {
  const limit = Math.min(12, Math.max(1, Number(context.requestedCount || 10) || 10));
  return list.slice(0, limit).map((product) => ({
    id: product.id,
...
function buildProductReply(list, context = {}) {
  ...
  return `Saya pilihkan ${names}.${budgetLine} Silakan lihat kartu produk di bawah ini dan cek detail sebelum checkout.`;
}
...
function buildNoProductReply(context = {}) {
  ...
  return `Saya belum menemukan produk ready yang benar-benar cocok${detail}. Coba naikkan budget, longgarkan kategori, atau hubungi admin untuk konfirmasi stok terbaru.`;
}

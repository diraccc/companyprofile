...
const EXTERNAL_QA_MAX_BYTES = Math.max(1024, Number(process.env.EXTERNAL_QA_MAX_BYTES || 25 * 1024 * 1024));
const EXTERNAL_QA_MAX_ENTRIES = Math.max(1, Number(process.env.EXTERNAL_QA_MAX_ENTRIES || 2000000));
const EXTERNAL_QA_FUZZY_LIMIT = Math.max(0, Number(process.env.EXTERNAL_QA_FUZZY_LIMIT || 50000));
const EXTERNAL_QA_MIN_SCORE = Math.max(0.5, Math.min(0.98, Number(process.env.EXTERNAL_QA_MIN_SCORE || 0.68)));
let EXTERNAL_QA_CACHE = { signature: '', entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: null, loadedAt: 0, source: '', files: [] };
  ...
  try {
    ...
    if (!hasProvider()) {
      ...
      return res.status(200).json(makeReply('fallback', finalFallbackText, {
        traceId,
        intent: intent.name,
        provider: 'external-qa-json-split',
        confidence: 0.35,
...
function externalQaFallbackReply(message, traceId, intentName, startedAt, reason) {
  ...
  return makeReply('conversation', match.answer, {
    ...
    externalQa: {
      source: match.source || '/data/qa-1.json ... /data/qa-50.json',
      status: 'matched',
      reason,
      matchType: match.matchType,
      matchedQuestion: match.question
    },
    analytics: { intent: intentName, source: 'external-qa-json-split', reason, ms: Date.now() - startedAt }
  });
}
...
function externalQaStatus() {
  ...
  return {
    source: loaded.source || '/data/qa-1.json ... /data/qa-50.json',
    files: loaded.files || [],
    status: loaded.error ? 'partial_or_not_loaded' : 'loaded',
    entries: loaded.count || loaded.entries.length,
...
function findExternalQaAnswer(message) {
  const loaded = loadExternalQaEntries();
  if (loaded.error && !(loaded.count || loaded.entries.length)) return null;
  if (!(loaded.count || loaded.entries.length)) return null;
  ...
  if (exactHit) {
    return {
      ...
      confidence: 1,
      matchType: 'exact',
      source: exactHit.entry.source || loaded.source || '/data/qa-1.json ... /data/qa-50.json'
    };
  }
  ...
  return {
    ...
    source: best.entry.source || loaded.source || '/data/qa-1.json ... /data/qa-50.json'
  };
}
...
function loadExternalQaEntries() {
  ...
  if (!files.length) {
    EXTERNAL_QA_CACHE = { signature: 'missing', entries: [], count: 0, indexes: { exact: new Map(), tokenIndex: new Map() }, error: 'File /data/qa-1.json sampai /data/qa-50.json belum ditemukan.', loadedAt: Date.now(), source: '/data/qa-1.json ... /data/qa-50.json', files: [] };
  ...
  try {
    for (const found of files) {
      const stat = fs.statSync(found.abs);
      parts.push(`${found.abs}:${stat.size}:${stat.mtimeMs}`);
    }
  } catch (error) {
    EXTERNAL_QA_CACHE = { signature: 'stat-error', entries: [], count: 0, indexes: { exact: new Map(), tokenIndex: new Map() }, error: 'Salah satu file QA belum bisa dibaca.', loadedAt: Date.now(), source: '/data/qa-1.json ... /data/qa-50.json', files: files.map((item) => item.publicPath) };
    return EXTERNAL_QA_CACHE;
  }
  ...
  try {
    for (const found of files) {
      if (totalEntries >= EXTERNAL_QA_MAX_ENTRIES) break;
      const stat = fs.statSync(found.abs);
      if (stat.size > EXTERNAL_QA_MAX_BYTES) {
        errors.push(`${found.publicPath} terlalu besar (${stat.size} byte)`);
        continue;
      }
      const raw = fs.readFileSync(found.abs, 'utf8').replace(/^\uFEFF/, '');
      if (!raw.trim()) continue;
      const parsed = JSON.parse(raw);
      const normalized = normalizeExternalQaEntries(parsed, found.publicPath);
      for (const entry of normalized) {
        if (totalEntries >= EXTERNAL_QA_MAX_ENTRIES) break;
        totalEntries += 1;
        for (const question of entry.questions) {
          const questionNorm = normalize(question);
          if (!questionNorm || exact.has(questionNorm)) continue;
          exact.set(questionNorm, { entry: Object.freeze({ answer: entry.answer, source: entry.source }), question });
        }
        if (fuzzyEntries.length < EXTERNAL_QA_FUZZY_LIMIT) fuzzyEntries.push(entry);
      }
    }

    const indexes = buildExternalQaIndexes(fuzzyEntries);
    for (const [key, value] of exact.entries()) {
      if (!indexes.exact.has(key)) indexes.exact.set(key, value);
    }
    ...
    EXTERNAL_QA_CACHE = {
      ...
      loadedAt: Date.now(),
      source: files.map((item) => item.publicPath).join(', '),
      files: files.map((item) => item.publicPath)
    };
    return EXTERNAL_QA_CACHE;
  } catch (error) {
    EXTERNAL_QA_CACHE = { signature, entries: [], count: 0, indexes: { exact: new Map(), tokenIndex: new Map() }, error: `File QA split tidak valid: ${sanitizeError(error)}`, loadedAt: Date.now(), source: files.map((item) => item.publicPath).join(', '), files: files.map((item) => item.publicPath) };
    return EXTERNAL_QA_CACHE;
  }
}
...
function resolveExternalQaFiles() {
  ...
  for (let i = 1; i <= EXTERNAL_QA_SPLIT_COUNT; i += 1) {
    ...
    for (const root of roots) {
      splitCandidates.push({ abs: path.join(root, 'data', filename), publicPath });
    }
  }
  ...
  for (const candidate of splitCandidates) {
    try {
      if (!candidate || !candidate.abs || seenPublic.has(candidate.publicPath)) continue;
      if (fs.existsSync(candidate.abs) && fs.statSync(candidate.abs).isFile()) {
        splitFiles.push(candidate);
        seenPublic.add(candidate.publicPath);
      }
    } catch (_) {}
  }
  if (splitFiles.length) return splitFiles.sort((a, b) => {
  ...
  if (configured) legacyCandidates.push({ abs: path.isAbsolute(configured) ? configured : path.join(process.cwd(), configured), publicPath: configured.startsWith('/') ? configured : `/${configured.replace(/^\.?\/?/, '')}` });
  legacyCandidates.push({ abs: path.join(process.cwd(), 'data', 'qa.json'), publicPath: '/data/qa.json' });
  legacyCandidates.push({ abs: path.join(__dirname, '..', 'data', 'qa.json'), publicPath: '/data/qa.json' });
  legacyCandidates.push({ abs: path.join(__dirname, 'data', 'qa.json'), publicPath: '/data/qa.json' });
  ...
  for (const candidate of legacyCandidates) {
    try {
      if (!candidate || !candidate.abs || seenAbs.has(candidate.abs)) continue;
      if (fs.existsSync(candidate.abs) && fs.statSync(candidate.abs).isFile()) {
        legacyFiles.push(candidate);
        seenAbs.add(candidate.abs);
      }
    } catch (_) {}
  }
  return legacyFiles.slice(0, 1);
}

function normalizeExternalQaEntries(parsed, source = '') {
  let rows = [];
  ...
  for (const row of rows) {
    ...
    entries.push(Object.freeze({ questions: Object.freeze(questions), answer, source }));
  }
  return entries;
}

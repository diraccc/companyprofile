...
const DIRAC_CHAT_QA_SPLIT_BACKEND_VERSION = 'DIRAC_CHAT_QA_SPLIT_BACKEND_V4_QA_1_1000_DIAGNOSTIC_TOTAL_QA_1_65_TYPO_1_5';
...
let EXTERNAL_QA_SPLIT_CACHE = { file: '', signature: '', parsed: null, error: null, publicPath: '' };
const QA_SPLIT_DIAGNOSTIC_MAX_FILE = Math.max(1, Math.min(1000, Number(process.env.QA_SPLIT_DIAGNOSTIC_MAX_FILE || 65)));
let QA_SPLIT_DIAGNOSTIC_CACHE = { signature: '', result: null, loadedAt: 0 };
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (isQaSplitDiagnosticMessage(normalizedMessage)) {
      return res.status(200).json(buildQaSplitDiagnosticReply(traceId, startedAt));
    }
    ...
    const primaryExternalQa = externalQaFallbackReply(message, traceId, intent.name, startedAt, 'split-qa-after-products');
    ...
    if (intent.name === 'general' && !hasProvider()) {
      const externalQaFileAnswer = externalQaFallbackReply(message, traceId, intent.name, startedAt, 'external-qa-file-match');
      if (externalQaFileAnswer) return res.status(200).json(externalQaFileAnswer);
    ...
    if (!hasProvider()) {
      const externalFallback = externalQaFallbackReply(message, traceId, intent.name, startedAt, 'no-provider');
      ...
      const finalFallbackText = looksLikeNoAnswer(fallbackText) ? buildExternalQaNoAnswerMessage() : fallbackText;
      return res.status(200).json(makeReply('fallback', finalFallbackText, {
        traceId,
        intent: intent.name,
        provider: 'external-qa-json',
        confidence: 0.35,
        externalQa: externalQaStatus(),
        analytics: { intent: intent.name, source: 'external-qa-json-miss', ms: Date.now() - startedAt }
      }));
    }
    ...
    try {
      ...
      const externalFallback = externalQaFallbackReply(message, traceId, intent.name, startedAt, 'provider-error');
      if (externalFallback) return res.status(200).json(externalFallback);
      return res.status(200).json(makeReply('fallback', buildExternalQaNoAnswerMessage(), {
        traceId,
        intent: intent.name,
        provider: 'external-qa-json',
        confidence: 0.35,
        externalQa: externalQaStatus(),
        analytics: { intent: intent.name, source: 'provider-error-external-qa-miss', error: sanitizeError(providerError), ms: Date.now() - startedAt }
      }));
    }

    const safeText = sanitizeAiText(ai.text, intent);
    if (shouldUseExternalQaAfterAi(safeText, intent, message)) {
      const externalFallback = externalQaFallbackReply(message, traceId, intent.name, startedAt, 'ai-no-answer');
      if (externalFallback) return res.status(200).json(externalFallback);
      return res.status(200).json(makeReply('fallback', buildExternalQaNoAnswerMessage(), {
        traceId,
        intent: intent.name,
        ...
        confidence: 0.35,
        externalQa: externalQaStatus(),
        analytics: { intent: intent.name, source: 'ai-no-answer-external-qa-miss', aiProvider: ai.provider, ms: Date.now() - startedAt }
      }));
    }
...
function externalQaFallbackReply(message, traceId, intentName, startedAt, reason) {
  ...
  return makeReply('conversation', match.answer, {
    ...
    analytics: { intent: intentName, source: 'external-qa-json', reason, ms: Date.now() - startedAt }
  });
}
...
function shouldUseExternalQaAfterAi(answerText, intent, userMessage) {
  ...
  if (intent && (intent.name === 'product_price' || intent.name === 'recommendation_ready' || intent.name === 'product_search' || intent.name === 'compare_products' || intent.name === 'checkout' || intent.name === 'tracking')) return false;
  if (!n) return true;
  if (/\b(maaf|sorry)\b.{0,80}\b(belum|tidak|ga|nggak|gak)\b.{0,80}\b(tahu|punya jawaban|menemukan jawaban|menjawab|data|informasi)\b/.test(n)) return true;
  if (/\b(saya|aku)\b.{0,60}\b(belum|tidak|ga|nggak|gak)\b.{0,80}\b(tahu|punya data|punya informasi|bisa menjawab|dapat menjawab|menemukan)\b/.test(n)) return true;
  if (/\b(data|informasi|jawaban)\b.{0,60}\b(belum tersedia|tidak tersedia|tidak ditemukan|belum ada)\b/.test(n)) return true;
  if (/\b(api key belum disetel|ai utama belum aktif|ai utama sedang tidak aktif|belum punya jawaban|ini pertanyaan umum|topik umum yang sangat luas|tulis pertanyaannya dengan lebih spesifik|tulis pertanyaan dengan lebih spesifik)\b/.test(n)) return true;
  return false;
}
...
function findExternalQaAnswer(message) {
  ...
  const exactHit = loaded.indexes && loaded.indexes.exact && loaded.indexes.exact.get(userNorm);
  if (exactHit) {
    return {
      answer: String(exactHit.entry.answer || '').trim().slice(0, 6000),
      question: String(exactHit.question || '').trim().slice(0, 300),
      confidence: 1,
      matchType: 'exact',
      source: loaded.source || '/data/qa.json'
    };
  ...
  for (const index of candidateIndexes) {
    ...
    for (const question of entry.questions) {
      ...
      if (!best || scored.score > best.score) {
        best = { entry, question, score: scored.score, matchType: scored.matchType };
  ...
  return {
    answer: String(best.entry.answer || '').trim().slice(0, 6000),
    question: String(best.question || best.entry.question || '').trim().slice(0, 300),
...
function preferredExternalQaFileNumbers(userNorm) {
  ...
  const socialMap = [
    [11, ['integrasi sosial','stratifikasi sosial','mobilitas sosial','sosialisasi','norma sosial','nilai sosial','institusi sosial','konflik sosial','perubahan sosial','solidaritas sosial','kontrol sosial','identitas sosial']],
    [12, ['ketidaksetaraan sosial','kemiskinan struktural','kelas sosial','diskriminasi','interseksionalitas','kapital sosial','eksklusi sosial','keadilan sosial','redistribusi','akses pendidikan','kesenjangan digital','privilege sosial']],
    [13, ['globalisasi','glokalisasi','homogenisasi budaya','hibridisasi budaya','migrasi internasional','diaspora','identitas budaya','akulturasi','asimilasi','pluralisme budaya','imperialisme budaya','komodifikasi budaya']],
    [14, ['demokrasi','partisipasi politik','kewarganegaraan','legitimasi kekuasaan','gerakan sosial','opini publik','kebijakan publik','hak asasi manusia','civil society','desentralisasi','representasi politik','akuntabilitas']],
    [15, ['ekonomi pasar','ekonomi campuran','kapitalisme','sosialisme','welfare state','tenaga kerja','ekonomi informal','kewirausahaan sosial','ekonomi berbagi','kemiskinan perkotaan','upah layak','ketahanan ekonomi keluarga']],
    [16, ['pendidikan inklusif','kurikulum tersembunyi','mobilitas pendidikan','literasi digital','pendidikan karakter','akses sekolah','kesenjangan pendidikan','pembelajaran jarak jauh','budaya sekolah','pendidikan kewargaan','pendidikan keluarga','pelatihan kerja']],
    [17, ['keluarga nuklir','keluarga luas','peran gender','pernikahan adat','pengasuhan anak','single parent','relasi keluarga','sosialisasi primer','domestic labor','kesetaraan gender','generasi muda','lansia dalam keluarga']],
    [18, ['media massa','literasi media','hoaks','filter bubble','komunikasi digital','jurnalisme warga','algoritma media sosial','privasi data','cyberbullying','budaya viral','influencer','partisipasi digital']],
    [19, ['deviasi sosial','labeling theory','anomie','kejahatan kerah putih','kontrol sosial formal','kontrol sosial informal','sistem peradilan','kriminalitas remaja','rehabilitasi sosial','restorative justice','stigma sosial','penyimpangan kolektif']],
    [20, ['lingkungan sosial','keadilan lingkungan','perubahan iklim','ketahanan komunitas','bencana sosial','adaptasi masyarakat','sampah perkotaan','air bersih','urban green space','ekologi politik','konflik sumber daya','konsumsi berkelanjutan']],
    [21, ['urbanisasi','demografi','bonus demografi','piramida penduduk','migrasi internal','kepadatan penduduk','penuaan penduduk','permukiman kumuh','transportasi publik','perencanaan kota','gentrifikasi','sprawl perkotaan']],
    [22, ['kesehatan masyarakat','determinan sosial kesehatan','akses layanan kesehatan','stigma kesehatan mental','perilaku hidup sehat','ketahanan pangan','vaksinasi publik','sanitasi','epidemi sosial','kesehatan reproduksi','dukungan komunitas','asuransi kesehatan']],
    [23, ['organisasi sosial','budaya organisasi','kepemimpinan','birokrasi','solidaritas kerja','konflik organisasi','etos kerja','komunikasi organisasi','kerja jarak jauh','serikat pekerja','manajemen perubahan','kesejahteraan pekerja']],
    [24, ['perdamaian sosial','mediasi konflik','resolusi konflik','rekonsiliasi','toleransi','moderasi sosial','dialog antaragama','kohesi sosial','radikalisasi','polarisasi sosial','kepercayaan sosial','trauma kolektif']],
    [25, ['metode survei','wawancara mendalam','observasi partisipan','etnografi','analisis wacana','validitas penelitian','reliabilitas data','sampling sosial','etika penelitian','triangulasi data','mixed methods','analisis kebijakan']]
  ];

  for (const [fileNumber, phrases] of socialMap) {
    if (phrases.some((phrase) => userNorm.includes(normalize(phrase)))) numbers.push(fileNumber);
  }

  return unique(numbers).filter((n) => Number.isFinite(n) && n >= 1 && n <= EXTERNAL_QA_SPLIT_COUNT);
}
...
function readExternalQaSplitJson(file) {
  try {
    ...
    if (EXTERNAL_QA_SPLIT_CACHE.signature === signature && EXTERNAL_QA_SPLIT_CACHE.parsed) {
      return { data: EXTERNAL_QA_SPLIT_CACHE.parsed, publicPath: file.publicPath };
    }

    if (stat.size > EXTERNAL_QA_SPLIT_MAX_BYTES) {
      return { error: `File ${file.publicPath} terlalu besar (${stat.size} byte).` };
    }

    const parsed = JSON.parse(fs.readFileSync(file.abs, 'utf8').replace(/^\uFEFF/, ''));
    EXTERNAL_QA_SPLIT_CACHE = { file: file.abs, signature, parsed, error: null, publicPath: file.publicPath };
    return { data: parsed, publicPath: file.publicPath };
  } catch (error) {
    return { error: `File ${file.publicPath} gagal dibaca: ${sanitizeError(error)}` };
  }
}


function resolveExternalQaSplitFileNumber(fileNumber) {
  const n = Number(fileNumber || 0);
  if (!Number.isFinite(n) || n < 1 || n > EXTERNAL_QA_SPLIT_COUNT) return null;

  const filename = `${EXTERNAL_QA_SPLIT_PREFIX}-${n}.json`;
  const publicPath = `/data/${filename}`;
  const roots = unique([process.cwd(), path.join(__dirname, '..'), __dirname]);
  ...
  for (const root of roots) {
    ...
    try {
      if (fs.existsSync(abs) && fs.statSync(abs).isFile()) {
        return { abs, publicPath, number: n };
      }
    } catch (_) {}
  }

  return { abs: '', publicPath, number: n, missing: true };
}

function isQaSplitDiagnosticMessage(normalizedText) {
  const n = String(normalizedText || '').trim();
  return n === 'cek qa split' || n === 'cek qasplit' || /\bcek\s+qa\s+split\b/.test(n) || /\bdiagnostic\s+qa\s+split\b/.test(n);
}

function countQuestionsInExternalQaParsed(parsed) {
  if (!parsed) return 0;
  ...
  if (Array.isArray(parsed)) {
    ...
    for (const row of parsed) {
      ...
      const hasQuestion = row.pertanyaan !== undefined || row.question !== undefined || row.q !== undefined || row.tanya !== undefined || row.prompt !== undefined || row.key !== undefined || row.keyword !== undefined;
      const hasAnswer = row.jawaban !== undefined || row.answer !== undefined || row.a !== undefined || row.respon !== undefined || row.response !== undefined || row.value !== undefined;
      if (hasQuestion || hasAnswer) total += 1;
    }
    return total;
  }
...
function inspectExternalQaSplitFileForDiagnostic(file) {
  if (!file || file.missing) {
    return { number: file ? file.number : 0, publicPath: file ? file.publicPath : '', exists: false, bytes: 0, entries: 0, error: 'missing' };
  }

  try {
    const stat = fs.statSync(file.abs);
    if (!stat.isFile()) {
      return { number: file.number, publicPath: file.publicPath, exists: false, bytes: 0, entries: 0, error: 'not_file' };
    }

    if (stat.size > EXTERNAL_QA_SPLIT_MAX_BYTES) {
      return { number: file.number, publicPath: file.publicPath, exists: true, bytes: stat.size, entries: 0, error: `too_large:${stat.size}` };
    }

    const raw = fs.readFileSync(file.abs, 'utf8').replace(/^\uFEFF/, '');
    const parsed = JSON.parse(raw);
    return { number: file.number, publicPath: file.publicPath, exists: true, bytes: stat.size, entries: countQuestionsInExternalQaParsed(parsed), error: null };
  } catch (error) {
    let bytes = 0;
    try { bytes = fs.statSync(file.abs).size || 0; } catch (_) {}
    return { number: file.number, publicPath: file.publicPath, exists: true, bytes, entries: 0, error: sanitizeError(error) };
  }
}

function getQaSplitDiagnostic(maxFile = QA_SPLIT_DIAGNOSTIC_MAX_FILE) {
  const limit = Math.max(1, Math.min(EXTERNAL_QA_SPLIT_COUNT, Number(maxFile || QA_SPLIT_DIAGNOSTIC_MAX_FILE)));
  const files = [];
  const signatureParts = [];

  for (let i = 1; i <= limit; i += 1) {
    const file = resolveExternalQaSplitFileNumber(i);
    files.push(file);
    if (!file || file.missing) {
      signatureParts.push(`${i}:missing`);
      continue;
    }

    try {
      const stat = fs.statSync(file.abs);
      signatureParts.push(`${i}:${stat.size}:${Math.round(stat.mtimeMs)}`);
    } catch (_) {
      signatureParts.push(`${i}:stat_error`);
    }
  }

  const signature = signatureParts.join('|');
  if (QA_SPLIT_DIAGNOSTIC_CACHE.signature === signature && QA_SPLIT_DIAGNOSTIC_CACHE.result) {
    return QA_SPLIT_DIAGNOSTIC_CACHE.result;
  }

  const inspected = files.map((file) => inspectExternalQaSplitFileForDiagnostic(file));
  const found = inspected.filter((item) => item.exists && !item.error);
  const missing = inspected.filter((item) => !item.exists);
  const errors = inspected.filter((item) => item.exists && item.error);
  const totalQuestions = found.reduce((sum, item) => sum + Number(item.entries || 0), 0);
  const totalBytes = found.reduce((sum, item) => sum + Number(item.bytes || 0), 0);
  const result = {
    checkedUntil: limit,
    ...
    errorFiles: errors.map((item) => ({ number: item.number, publicPath: item.publicPath, error: item.error, bytes: item.bytes })),
    loadedAt: new Date().toISOString()
  };

  QA_SPLIT_DIAGNOSTIC_CACHE = { signature, result, loadedAt: Date.now() };
  return result;
}

function formatNumberCompact(number) {
  return String(Number(number || 0)).replace(/\B(?=(\d{3})+(?!\d))/g, '.');
}

function formatQaNumberList(numbers, limit = 80) {
  const list = Array.isArray(numbers) ? numbers : [];
  if (!list.length) return '-';
  const safeLimit = Math.max(1, Number(limit || 80));
  if (list.length <= safeLimit) return list.join(', ');
  return list.slice(0, safeLimit).join(', ') + ` ... +${list.length - safeLimit} lagi`;
}

function buildQaSplitDiagnosticText(diagnostic) {
  const foundNumbers = diagnostic.files.filter((item) => item.exists && !item.error).map((item) => item.number);
  const lines = [
    `${DIRAC_CHAT_QA_SPLIT_BACKEND_VERSION} aktif di chat.js.`,
    `Cek QA split: qa-1.json sampai qa-${diagnostic.checkedUntil}.json.`,
    `File ditemukan normal: ${diagnostic.foundCount}/${diagnostic.checkedUntil}.`,
    `Total pertanyaan terbaca: ${formatNumberCompact(diagnostic.totalQuestions)}.`,
    `Total ukuran file normal: ${formatNumberCompact(diagnostic.totalBytes)} bytes.`,
    `File ditemukan: ${formatQaNumberList(foundNumbers, 80)}.`
  ];

  if (diagnostic.missingCount > 0) {
    lines.push(`File belum ditemukan: ${formatQaNumberList(diagnostic.missingNumbers, 80)}.`);
  } else {
    lines.push('File belum ditemukan: tidak ada.');
  }

  if (diagnostic.errorCount > 0) {
    const errorText = diagnostic.errorFiles.slice(0, 12).map((item) => `qa-${item.number}: ${item.error}`).join(' | ');
    lines.push(`File error: ${errorText}${diagnostic.errorFiles.length > 12 ? ` | +${diagnostic.errorFiles.length - 12} error lagi` : ''}.`);
  } else {
    lines.push('File error: tidak ada.');
  }

  lines.push('Catatan: angka total pertanyaan dihitung dari isi JSON yang valid, bukan dari perkiraan nama file.');
  return lines.join('\n');
}

function buildQaSplitDiagnosticReply(traceId, startedAt) {
  const diagnostic = getQaSplitDiagnostic(QA_SPLIT_DIAGNOSTIC_MAX_FILE);
  return makeReply('conversation', buildQaSplitDiagnosticText(diagnostic), {
    traceId,
    intent: 'qa_split_diagnostic',
    provider: 'local-qa-split-diagnostic',
    confidence: diagnostic.errorCount ? 0.8 : 0.99,
    externalQa: {
      status: diagnostic.errorCount ? 'partial' : 'loaded',
      checkedUntil: diagnostic.checkedUntil,
      splitCount: diagnostic.foundCount,
      totalQuestions: diagnostic.totalQuestions,
      missingCount: diagnostic.missingCount,
      errorCount: diagnostic.errorCount,
      missingNumbers: diagnostic.missingNumbers,
      errorFiles: diagnostic.errorFiles
    },
    analytics: { intent: 'qa_split_diagnostic', source: 'local-qa-split-diagnostic', ms: Date.now() - startedAt }
  });
}
...
function exactExternalQaRowHit(row, userNorm, allowTypo = false) {
  ...
  const questionSource = row.pertanyaan ?? row.question ?? row.q ?? row.tanya ?? row.prompt ?? row.key ?? row.keyword;
  const questionList = [];
  if (Array.isArray(questionSource)) questionList.push(...questionSource);
  else if (questionSource) questionList.push(questionSource);
  for (const key of ['alias','aliases','keywords','kataKunci','kata_kunci','pertanyaanAlternatif','questions']) {
    const value = row[key];
...
function scoreExternalQaQuestion(userNorm, questionNorm) {
  ...
  if (userNorm.length >= 8 && questionNorm.includes(userNorm)) return { score: 0.96, matchType: 'question-contains-user' };
  if (questionNorm.length >= 8 && userNorm.includes(questionNorm)) return { score: 0.94, matchType: 'user-contains-question' };

  const userTokens = externalQaTokens(userNorm);
  const questionTokens = externalQaTokens(questionNorm);
  if (!userTokens.length || !questionTokens.length) return { score: 0, matchType: 'none' };
  ...
  let score = (userCoverage * 0.45) + (questionCoverage * 0.45) + (jaccard * 0.10);

  const userPhrase = userTokens.join(' ');
  const questionPhrase = questionTokens.join(' ');
  if (userPhrase.length >= 6 && questionPhrase.includes(userPhrase)) score += 0.08;
  if (questionPhrase.length >= 6 && userPhrase.includes(questionPhrase)) score += 0.08;
...
function candidateExternalQaIndexes(userNorm, loaded) {
  ...
  if (!loaded.indexes || !loaded.indexes.tokenIndex) return Array.from({ length: allFallbackLimit }, (_, index) => index);
  ...
  if (!counts.size) return Array.from({ length: allFallbackLimit }, (_, index) => index);
...
function buildExternalQaIndexes(entries) {
  ...
  entries.forEach((entry, index) => {
    for (const question of entry.questions) {
      const questionNorm = normalize(question);
      if (!questionNorm) continue;
      if (!exact.has(questionNorm)) exact.set(questionNorm, { entry, question });
...
function loadExternalQaEntries() {
  ...
  if (!found) {
    EXTERNAL_QA_CACHE = { signature: 'missing', entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: 'File QA belum ditemukan.', loadedAt: Date.now(), source: '/data/qa.json' };
  ...
  try {
    ...
    EXTERNAL_QA_CACHE = { signature: 'stat-error', entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: 'File QA belum bisa dibaca.', loadedAt: Date.now(), source: found };
    return EXTERNAL_QA_CACHE;
  }
  ...
  try {
    if (stat.size > EXTERNAL_QA_MAX_BYTES) {
      EXTERNAL_QA_CACHE = { signature, entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: `File QA terlalu besar (${stat.size} byte).`, loadedAt: Date.now(), source: found };
      return EXTERNAL_QA_CACHE;
    }
    const raw = fs.readFileSync(found, 'utf8').replace(/^\uFEFF/, '');
    const parsed = JSON.parse(raw);
    const entries = normalizeExternalQaEntries(parsed).slice(0, EXTERNAL_QA_MAX_ENTRIES);
    const indexes = buildExternalQaIndexes(entries);
    EXTERNAL_QA_CACHE = { signature, entries, indexes, error: null, loadedAt: Date.now(), source: found };
    return EXTERNAL_QA_CACHE;
  } catch (error) {
    EXTERNAL_QA_CACHE = { signature, entries: [], indexes: { exact: new Map(), tokenIndex: new Map() }, error: `File QA tidak valid: ${sanitizeError(error)}`, loadedAt: Date.now(), source: found };
    return EXTERNAL_QA_CACHE;
  }
}
...
function resolveExternalQaFile() {
  ...
  if (configured) candidates.push(path.isAbsolute(configured) ? configured : path.join(process.cwd(), configured));
...
function normalizeExternalQaEntries(parsed) {
  ...
  else if (parsed && typeof parsed === 'object') rows = Object.entries(parsed).map(([question, answer]) => ({ question, answer }));
  ...
  for (const row of rows) {
    ...
    const questions = unique(questionList.map((item) => String(item || '').trim()).filter(Boolean));
    if (!questions.length) continue;
    entries.push(Object.freeze({ questions: Object.freeze(questions), answer }));
  }
  return entries;
}
...
const DIRAC_ULTRA_100K_CONCEPTS = Object.freeze([{"topic":"kecerdasan buatan lanjutan","aliases":["ai lanjutan","artificial intelligence advanced","sistem ai"],"category":"teknologi","answer":"Kecerdasan buatan lanjutan adalah penggunaan model dan algoritma untuk memahami pola, membuat prediksi, menyusun bahasa, mengenali gambar, dan membantu keputusan. Hasil AI perlu diperiksa karena bisa salah, bias, atau kurang konteks."},{"topic":"machine learning terawasi","aliases":["supervised learning","pembelajaran terawasi"],"category":"teknologi","answer":"Machine learning terawasi adalah metode belajar dari data berlabel. Model diberi contoh input dan jawaban benar, lalu belajar memetakan pola agar bisa memprediksi data baru."},{"topic":"machine learning tidak terawasi","aliases":["unsupervised learning","pembelajaran tidak terawasi"],"category":"teknologi","answer":"Machine learning tidak terawasi mencari pola dari data tanpa label jawaban. Contohnya pengelompokan pelanggan, deteksi kemiripan, dan reduksi dimensi."},{"topic":"reinforcement learning","aliases":["pembelajaran penguatan"],"category":"teknologi","answer":"Reinforcement learning adalah metode belajar melalui hadiah dan penalti. Agen mencoba tindakan, melihat hasilnya, lalu memperbaiki strategi agar memperoleh nilai terbaik."},{"topic":"large language model","aliases":["llm","model bahasa besar"],"category":"teknologi","answer":"Large language model adalah model AI yang dilatih pada banyak teks untuk memprediksi dan menyusun bahasa. LLM berguna untuk menjawab, merangkum, menulis, menerjemahkan, dan membantu analisis teks."},{"topic":"prompt engineering","aliases":["rekayasa prompt","prompting ai"],"category":"teknologi","answer":"Prompt engineering adalah cara menyusun instruksi yang jelas untuk AI agar jawaban lebih tepat. Prompt yang baik berisi tujuan, konteks, batasan, format keluaran, dan contoh bila diperlukan."},{"topic":"retrieval augmented generation","aliases":["rag","retrieval augmented generation"],"category":"teknologi","answer":"Retrieval augmented generation adalah teknik yang membuat AI mencari dokumen relevan lebih dulu, lalu menyusun jawaban berdasarkan sumber tersebut. Cara ini membantu mengurangi jawaban mengarang."},{"topic":"embedding","aliases":["vektor embedding","text embedding"],"category":"teknologi","answer":"Embedding adalah representasi data seperti teks atau gambar menjadi angka vektor. Vektor ini memudahkan komputer menghitung kemiripan makna, pencarian semantik, dan rekomendasi."},{"topic":"natural language processing","aliases":["nlp","pemrosesan bahasa alami"],"category":"teknologi","answer":"Natural language processing adalah bidang AI yang membuat komputer memahami, mengolah, dan menghasilkan bahasa manusia, misalnya chatbot, terjemahan, klasifikasi teks, dan ringkasan."},{"topic":"computer vision","aliases":["visi komputer"],"category":"teknologi","answer":"Computer vision adalah bidang AI yang membuat komputer memahami gambar atau video, seperti mengenali objek, membaca teks pada gambar, mendeteksi wajah, atau menganalisis kualitas visual."},{"topic":"data science","aliases":["ilmu data"],"category":"teknologi","answer":"Data science adalah gabungan statistik, pemrograman, pemahaman bisnis, dan visualisasi untuk mengubah data menjadi wawasan, prediksi, atau rekomendasi keputusan."},{"topic":"data cleaning","aliases":["pembersihan data"],"category":"teknologi","answer":"Data cleaning adalah proses memperbaiki data yang duplikat, kosong, salah format, tidak konsisten, atau mengandung nilai ekstrem agar analisis lebih akurat."},{"topic":"model evaluation","aliases":["evaluasi model","metrik model"],"category":"teknologi","answer":"Evaluasi model adalah proses mengukur kinerja model dengan metrik yang sesuai, seperti akurasi, precision, recall, F1, error rata-rata, atau validasi silang."},{"topic":"overfitting","aliases":["model overfit"],"category":"teknologi","answer":"Overfitting terjadi saat model terlalu hafal data latihan sehingga buruk pada data baru. Pencegahannya antara lain data validasi, regularisasi, model lebih sederhana, dan data lebih beragam."},{"topic":"bias algoritma","aliases":["algorithmic bias","bias ai"],"category":"teknologi","answer":"Bias algoritma terjadi ketika sistem menghasilkan keputusan tidak adil karena data, desain, atau evaluasinya berat sebelah. Perlu audit data, uji dampak, dan pengawasan manusia."},{"topic":"privasi data","aliases":["data privacy","perlindungan data pribadi"],"category":"teknologi","answer":"Privasi data adalah perlindungan informasi pribadi agar dikumpulkan, diproses, dan dibagikan secara sah, aman, terbatas, dan sesuai persetujuan."},{"topic":"keamanan siber lanjutan","aliases":["cyber security advanced","keamanan digital"],"category":"teknologi","answer":"Keamanan siber lanjutan mencakup pencegahan, deteksi, respons, dan pemulihan dari serangan digital. Praktiknya meliputi update sistem, autentikasi kuat, enkripsi, monitoring, dan edukasi pengguna."},{"topic":"autentikasi dua faktor","aliases":["2fa","mfa","two factor authentication"],"category":"teknologi","answer":"Autentikasi dua faktor menambah lapisan keamanan selain password, misalnya kode OTP, aplikasi autentikator, kunci keamanan, atau biometrik."},{"topic":"phishing lanjutan","aliases":["social engineering digital","penipuan phishing"],"category":"teknologi","answer":"Phishing lanjutan menyamar sangat meyakinkan melalui email, chat, iklan, atau website palsu. Tanda bahaya: domain aneh, tekanan waktu, permintaan OTP, dan lampiran tidak jelas."},{"topic":"api rest","aliases":["rest api","api restful"],"category":"teknologi","answer":"REST API adalah cara layanan web berkomunikasi memakai URL, metode HTTP seperti GET dan POST, serta format data seperti JSON. API memisahkan frontend dari backend."},{"topic":"graphql","aliases":["graphql api"],"category":"teknologi","answer":"GraphQL adalah cara mengambil data dari API dengan query yang menentukan field yang dibutuhkan. Kelebihannya fleksibel, tetapi perlu desain skema dan keamanan yang baik."},{"topic":"frontend","aliases":["pengembangan frontend"],"category":"teknologi","answer":"Frontend adalah bagian aplikasi yang dilihat dan digunakan pengguna, biasanya dibuat dengan HTML, CSS, JavaScript, dan framework antarmuka."},{"topic":"backend","aliases":["pengembangan backend"],"category":"teknologi","answer":"Backend adalah bagian server yang mengelola logika bisnis, database, autentikasi, API, keamanan, dan integrasi layanan."},{"topic":"database relasional","aliases":["sql database","basis data relasional"],"category":"teknologi","answer":"Database relasional menyimpan data dalam tabel yang saling berhubungan. SQL dipakai untuk membuat, membaca, memperbarui, dan menganalisis data."},{"topic":"database nosql","aliases":["nosql","basis data nosql"],"category":"teknologi","answer":"Database NoSQL menyimpan data non-tabel seperti dokumen, key-value, graph, atau kolom lebar. Cocok untuk kebutuhan fleksibel, skala besar, atau data semi-terstruktur."},{"topic":"cache","aliases":["caching","memori cache"],"category":"teknologi","answer":"Cache menyimpan data sementara agar akses berikutnya lebih cepat. Cache dapat mempercepat website, API, database, dan aplikasi mobile."},{"topic":"cdn","aliases":["content delivery network"],"category":"teknologi","answer":"CDN adalah jaringan server yang menyebarkan konten ke lokasi dekat pengguna. Tujuannya mempercepat loading, mengurangi beban server utama, dan meningkatkan stabilitas."},{"topic":"serverless","aliases":["komputasi serverless"],"category":"teknologi","answer":"Serverless adalah model cloud di mana pengembang menjalankan fungsi tanpa mengelola server langsung. Penyedia cloud mengurus skalabilitas dan infrastruktur."},{"topic":"git","aliases":["version control git"],"category":"teknologi","answer":"Git adalah sistem version control untuk melacak perubahan kode, membuat cabang kerja, menggabungkan perubahan, dan memudahkan kolaborasi tim."},{"topic":"ui ux","aliases":["desain ui ux","user interface user experience"],"category":"teknologi","answer":"UI/UX membahas tampilan dan pengalaman pengguna. UI fokus pada elemen visual, sedangkan UX fokus pada kemudahan, alur, kebutuhan pengguna, dan kepuasan penggunaan."},{"topic":"seo","aliases":["search engine optimization","optimasi mesin pencari"],"category":"teknologi","answer":"SEO adalah upaya membuat halaman lebih mudah ditemukan mesin pencari melalui konten relevan, struktur teknis baik, kecepatan, tautan, dan pengalaman pengguna."},{"topic":"web analytics","aliases":["analitik website","analytics digital"],"category":"teknologi","answer":"Web analytics mengukur perilaku pengunjung seperti sumber trafik, halaman populer, durasi kunjungan, konversi, dan hambatan pengguna untuk memperbaiki strategi digital."},{"topic":"testing software","aliases":["pengujian software","software testing"],"category":"teknologi","answer":"Testing software adalah proses memeriksa apakah aplikasi berjalan sesuai harapan. Bentuknya bisa unit test, integration test, end-to-end test, dan regression test."},{"topic":"accessibility web","aliases":["aksesibilitas web","a11y"],"category":"teknologi","answer":"Aksesibilitas web memastikan website dapat dipakai banyak orang, termasuk pengguna pembaca layar, keyboard, kontras tinggi, atau kebutuhan khusus lain."},{"topic":"progressive web app","aliases":["pwa","aplikasi web progresif"],"category":"teknologi","answer":"Progressive Web App adalah website yang terasa seperti aplikasi, dapat responsif, cepat, bisa memakai service worker, dan kadang mendukung offline atau instalasi."},{"topic":"internet of things","aliases":["iot","internet benda"],"category":"teknologi","answer":"Internet of Things adalah jaringan perangkat fisik yang terhubung internet, seperti sensor, kamera, perangkat rumah pintar, dan mesin industri."},{"topic":"robotika","aliases":["robotics"],"category":"teknologi","answer":"Robotika adalah bidang yang merancang robot untuk merasakan lingkungan, mengambil keputusan, dan melakukan aksi fisik melalui sensor, kontrol, dan aktuator."},{"topic":"komputasi kuantum","aliases":["quantum computing"],"category":"teknologi","answer":"Komputasi kuantum memakai prinsip kuantum seperti superposisi dan entanglement untuk memproses informasi. Teknologi ini berpotensi kuat untuk masalah tertentu, tetapi masih berkembang."},{"topic":"blockchain lanjutan","aliases":["teknologi blockchain"],"category":"teknologi","answer":"Blockchain adalah buku besar digital yang dicatat berurutan dan disalin ke banyak node. Kekuatan utamanya transparansi, audit, dan ketahanan perubahan data."},{"topic":"smart contract","aliases":["kontrak pintar"],"category":"teknologi","answer":"Smart contract adalah program yang berjalan di blockchain untuk mengeksekusi aturan otomatis. Risikonya perlu audit karena kesalahan kode bisa berdampak finansial."},{"topic":"bilangan prima","aliases":["prime number","angka prima"],"category":"matematika","answer":"Bilangan prima adalah bilangan bulat lebih dari 1 yang hanya memiliki dua faktor positif, yaitu 1 dan dirinya sendiri, seperti 2, 3, 5, 7, dan 11."},{"topic":"pecahan","aliases":["fraction","bilangan pecahan"],"category":"matematika","answer":"Pecahan menyatakan bagian dari keseluruhan dalam bentuk pembilang dan penyebut. Contoh 3/4 berarti tiga bagian dari empat bagian sama besar."},{"topic":"persen","aliases":["persentase","percentage"],"category":"matematika","answer":"Persen berarti per seratus. Misalnya 25% sama dengan 25 dari 100 atau 0,25. Persen sering dipakai untuk diskon, bunga, perbandingan, dan statistik."},{"topic":"rasio","aliases":["perbandingan rasio"],"category":"matematika","answer":"Rasio adalah perbandingan dua besaran. Jika rasio gula dan air 1:4, artinya setiap 1 bagian gula dibandingkan dengan 4 bagian air."},{"topic":"aljabar dasar","aliases":["basic algebra"],"category":"matematika","answer":"Aljabar dasar memakai simbol atau variabel untuk mewakili bilangan. Dengan aljabar, kita bisa menyusun persamaan dan mencari nilai yang belum diketahui."},{"topic":"persamaan linear","aliases":["linear equation"],"category":"matematika","answer":"Persamaan linear adalah persamaan berpangkat satu, misalnya 2x + 3 = 7. Grafiknya berupa garis lurus dan solusinya bisa dicari dengan operasi aljabar."},{"topic":"fungsi matematika","aliases":["fungsi","mathematical function"],"category":"matematika","answer":"Fungsi adalah aturan yang memasangkan setiap input dengan tepat satu output. Contoh f(x)=2x+1 menghasilkan nilai berbeda sesuai x."},{"topic":"grafik fungsi","aliases":["graph of function"],"category":"matematika","answer":"Grafik fungsi menggambarkan hubungan input dan output pada bidang koordinat. Grafik membantu melihat pola naik, turun, titik potong, dan bentuk hubungan."},{"topic":"geometri","aliases":["geometry"],"category":"matematika","answer":"Geometri mempelajari bentuk, ukuran, posisi, sudut, luas, volume, dan hubungan ruang pada objek dua dimensi maupun tiga dimensi."},{"topic":"teorema pythagoras","aliases":["pythagorean theorem","rumus pythagoras"],"category":"matematika","answer":"Teorema Pythagoras menyatakan pada segitiga siku-siku, kuadrat sisi miring sama dengan jumlah kuadrat dua sisi lainnya: a² + b² = c²."},{"topic":"trigonometri","aliases":["trigonometry"],"category":"matematika","answer":"Trigonometri mempelajari hubungan sudut dan sisi segitiga, terutama fungsi sinus, cosinus, dan tangen. Bidang ini berguna dalam fisika, teknik, navigasi, dan grafik."},{"topic":"statistika deskriptif","aliases":["descriptive statistics"],"category":"matematika","answer":"Statistika deskriptif merangkum data dengan ukuran seperti rata-rata, median, modus, rentang, varians, simpangan baku, tabel, dan grafik."},{"topic":"probabilitas","aliases":["peluang","probability"],"category":"matematika","answer":"Probabilitas adalah ukuran kemungkinan suatu peristiwa terjadi, dari 0 berarti mustahil sampai 1 berarti pasti."},{"topic":"kombinatorika","aliases":["combinatorics"],"category":"matematika","answer":"Kombinatorika mempelajari cara menghitung susunan, pilihan, kombinasi, dan permutasi tanpa harus mendaftar semua kemungkinan satu per satu."},{"topic":"kalkulus dasar","aliases":["basic calculus"],"category":"matematika","answer":"Kalkulus dasar mempelajari perubahan dan akumulasi. Turunan membahas laju perubahan, sedangkan integral membahas penjumlahan kontinu atau luas di bawah kurva."},{"topic":"turunan","aliases":["derivative","diferensial"],"category":"matematika","answer":"Turunan mengukur laju perubahan suatu fungsi terhadap variabelnya. Dalam grafik, turunan berkaitan dengan kemiringan garis singgung."},{"topic":"integral","aliases":["integration","integral matematika"],"category":"matematika","answer":"Integral menjumlahkan perubahan kecil secara kontinu. Integral sering dipakai untuk menghitung luas, volume, total akumulasi, dan model fisika."},{"topic":"matriks","aliases":["matrix matematika"],"category":"matematika","answer":"Matriks adalah susunan bilangan dalam baris dan kolom. Matriks dipakai dalam sistem persamaan, transformasi geometri, komputer grafis, statistik, dan AI."},{"topic":"logika matematika","aliases":["mathematical logic"],"category":"matematika","answer":"Logika matematika mempelajari pernyataan benar-salah, implikasi, negasi, kuantor, dan aturan penalaran yang konsisten."},{"topic":"optimisasi","aliases":["optimization matematika"],"category":"matematika","answer":"Optimisasi mencari nilai terbaik berdasarkan tujuan dan batasan, misalnya biaya paling rendah, keuntungan terbesar, atau rute paling efisien."},{"topic":"atom lanjutan","aliases":["struktur atom"],"category":"sains","answer":"Atom tersusun dari inti berisi proton dan neutron, serta elektron di sekitar inti. Susunan partikel ini menentukan sifat unsur dan cara atom berikatan."},{"topic":"molekul lanjutan","aliases":["struktur molekul"],"category":"sains","answer":"Molekul terbentuk dari dua atau lebih atom yang terikat. Bentuk dan jenis ikatan memengaruhi sifat molekul seperti titik didih, kelarutan, dan reaktivitas."},{"topic":"tabel periodik","aliases":["periodic table"],"category":"sains","answer":"Tabel periodik menyusun unsur berdasarkan nomor atom dan kemiripan sifat. Kolom menunjukkan golongan, sedangkan baris menunjukkan periode."},{"topic":"ikatan kimia","aliases":["chemical bond"],"category":"sains","answer":"Ikatan kimia adalah gaya yang menyatukan atom. Jenis utamanya meliputi ikatan ion, kovalen, dan logam, masing-masing dengan cara berbagi atau memindahkan elektron."},{"topic":"asam basa","aliases":["acid base","larutan asam basa"],"category":"sains","answer":"Asam cenderung melepaskan ion hidrogen, sedangkan basa cenderung menerima ion hidrogen atau menghasilkan ion hidroksida. pH mengukur tingkat keasaman atau kebasaan."},{"topic":"reaksi kimia","aliases":["chemical reaction"],"category":"sains","answer":"Reaksi kimia adalah proses perubahan zat menjadi zat baru melalui pemutusan dan pembentukan ikatan. Tanda reaksi bisa berupa gas, endapan, panas, cahaya, atau perubahan warna."},{"topic":"sel hewan dan tumbuhan","aliases":["perbedaan sel hewan dan tumbuhan"],"category":"sains","answer":"Sel tumbuhan memiliki dinding sel, kloroplas, dan vakuola besar. Sel hewan tidak memiliki dinding sel dan kloroplas, tetapi memiliki organel lain untuk fungsi hidup."},{"topic":"dna dan rna","aliases":["dna rna"],"category":"sains","answer":"DNA menyimpan instruksi genetik utama, sedangkan RNA membantu menyalin dan menerjemahkan informasi genetik menjadi protein."},{"topic":"genetika dasar","aliases":["genetics basic"],"category":"sains","answer":"Genetika dasar mempelajari pewarisan sifat dari orang tua ke keturunan melalui gen, kromosom, variasi, mutasi, dan kombinasi alel."},{"topic":"fotosintesis lanjutan","aliases":["proses fotosintesis"],"category":"sains","answer":"Fotosintesis memakai cahaya, air, dan karbon dioksida untuk menghasilkan glukosa dan oksigen. Proses ini terjadi terutama di kloroplas tumbuhan."},{"topic":"respirasi sel","aliases":["cellular respiration"],"category":"sains","answer":"Respirasi sel adalah proses mengubah glukosa menjadi energi ATP. Pada organisme aerob, proses ini memakai oksigen dan menghasilkan karbon dioksida serta air."},{"topic":"ekosistem lanjutan","aliases":["hubungan ekosistem"],"category":"sains","answer":"Ekosistem terdiri dari makhluk hidup dan faktor fisik seperti air, tanah, cahaya, dan suhu. Keseimbangannya dipengaruhi aliran energi dan daur materi."},{"topic":"keanekaragaman hayati","aliases":["biodiversity"],"category":"sains","answer":"Keanekaragaman hayati adalah variasi gen, spesies, dan ekosistem. Semakin tinggi keanekaragaman, semakin kuat ekosistem menghadapi perubahan."},{"topic":"evolusi lanjutan","aliases":["seleksi alam"],"category":"sains","answer":"Evolusi terjadi saat sifat populasi berubah dari generasi ke generasi. Seleksi alam membuat sifat yang membantu bertahan hidup lebih mungkin diwariskan."},{"topic":"hukum newton","aliases":["newton laws","hukum gerak newton"],"category":"fisika","answer":"Hukum Newton menjelaskan gerak: benda mempertahankan keadaan jika resultan gaya nol, percepatan sebanding gaya, dan setiap aksi memiliki reaksi sama besar berlawanan arah."},{"topic":"energi kinetik","aliases":["kinetic energy"],"category":"fisika","answer":"Energi kinetik adalah energi karena gerak. Besarnya bergantung pada massa dan kecepatan benda, dirumuskan Ek = 1/2 mv²."},{"topic":"energi potensial","aliases":["potential energy"],"category":"fisika","answer":"Energi potensial adalah energi yang tersimpan karena posisi atau keadaan, misalnya benda di ketinggian memiliki energi potensial gravitasi."},{"topic":"listrik statis","aliases":["static electricity"],"category":"fisika","answer":"Listrik statis terjadi karena penumpukan muatan listrik pada permukaan benda. Contohnya rambut berdiri setelah digosok balon."},{"topic":"arus listrik","aliases":["electric current"],"category":"fisika","answer":"Arus listrik adalah aliran muatan dalam rangkaian. Arus mengalir jika ada beda potensial dan jalur tertutup."},{"topic":"hukum ohm","aliases":["ohms law","rumus ohm"],"category":"fisika","answer":"Hukum Ohm menyatakan tegangan sama dengan arus dikali hambatan: V = I × R. Rumus ini digunakan pada rangkaian listrik sederhana."},{"topic":"gelombang bunyi","aliases":["sound wave"],"category":"fisika","answer":"Gelombang bunyi adalah getaran yang merambat melalui medium seperti udara, air, atau benda padat. Bunyi tidak merambat di ruang hampa."},{"topic":"cahaya dan optik","aliases":["optik","light optics"],"category":"fisika","answer":"Optik mempelajari perilaku cahaya, termasuk pemantulan, pembiasan, lensa, bayangan, warna, dan alat optik seperti kamera atau mikroskop."},{"topic":"termodinamika dasar","aliases":["basic thermodynamics"],"category":"fisika","answer":"Termodinamika mempelajari panas, kerja, energi, suhu, dan perubahan keadaan. Prinsipnya dipakai pada mesin, kulkas, atmosfer, dan proses biologis."},{"topic":"tata surya","aliases":["solar system"],"category":"sains","answer":"Tata surya terdiri dari Matahari, planet, satelit, asteroid, komet, dan benda kecil lain yang terikat gravitasi Matahari."},{"topic":"bintang","aliases":["star astronomy"],"category":"sains","answer":"Bintang adalah bola gas panas yang memancarkan cahaya karena reaksi fusi nuklir di intinya. Matahari adalah bintang terdekat bagi Bumi."},{"topic":"galaksi","aliases":["galaxy"],"category":"sains","answer":"Galaksi adalah kumpulan besar bintang, gas, debu, planet, dan materi gelap yang terikat gravitasi. Bima Sakti adalah galaksi tempat Tata Surya berada."},{"topic":"lempeng tektonik","aliases":["plate tectonics"],"category":"sains","answer":"Lempeng tektonik adalah bagian kerak Bumi yang bergerak perlahan. Pergerakannya dapat membentuk gunung, gempa, palung, dan aktivitas vulkanik."},{"topic":"siklus air","aliases":["daur air","water cycle"],"category":"sains","answer":"Siklus air adalah perputaran air melalui penguapan, kondensasi, awan, hujan, aliran permukaan, infiltrasi, dan kembali ke laut atau atmosfer."},{"topic":"cuaca dan iklim","aliases":["weather climate"],"category":"sains","answer":"Cuaca adalah kondisi atmosfer jangka pendek, sedangkan iklim adalah pola rata-rata cuaca dalam jangka panjang di suatu wilayah."},{"topic":"bahasa indonesia baku","aliases":["bahasa baku","indonesia formal"],"category":"bahasa","answer":"Bahasa Indonesia baku mengikuti kaidah ejaan, tata bahasa, dan pilihan kata resmi. Bahasa baku dipakai dalam dokumen formal, pendidikan, berita, dan komunikasi resmi."},{"topic":"kalimat efektif lanjutan","aliases":["kalimat efektif jelas"],"category":"bahasa","answer":"Kalimat efektif menyampaikan gagasan secara jelas, hemat kata, logis, tidak ambigu, dan sesuai kaidah. Tujuannya agar pembaca cepat memahami maksud."},{"topic":"paragraf deduktif","aliases":["deductive paragraph"],"category":"bahasa","answer":"Paragraf deduktif menempatkan gagasan utama di awal paragraf, lalu diikuti kalimat penjelas yang memperkuat atau memberi contoh."},{"topic":"paragraf induktif","aliases":["inductive paragraph"],"category":"bahasa","answer":"Paragraf induktif menempatkan gagasan utama di akhir paragraf setelah beberapa kalimat penjelas atau contoh."},{"topic":"ide pokok","aliases":["gagasan utama"],"category":"bahasa","answer":"Ide pokok adalah inti pembahasan dalam paragraf. Ide pokok dapat ditemukan dengan melihat kalimat utama dan informasi yang paling sering didukung kalimat lain."},{"topic":"sinonim lanjutan","aliases":["padanan kata"],"category":"bahasa","answer":"Sinonim adalah kata yang maknanya mirip atau sama. Pemilihan sinonim harus memperhatikan konteks karena tidak semua kata mirip dapat saling menggantikan."},{"topic":"antonim lanjutan","aliases":["lawan kata"],"category":"bahasa","answer":"Antonim adalah kata yang maknanya berlawanan. Contohnya tinggi-rendah, cepat-lambat, dan terang-gelap."},{"topic":"majas","aliases":["gaya bahasa"],"category":"bahasa","answer":"Majas adalah gaya bahasa untuk membuat ungkapan lebih hidup, indah, atau kuat. Contohnya metafora, personifikasi, hiperbola, dan simile."},{"topic":"puisi","aliases":["poetry"],"category":"bahasa","answer":"Puisi adalah karya sastra yang menonjolkan pilihan kata, irama, citraan, dan makna padat untuk menyampaikan perasaan atau gagasan."},{"topic":"cerpen","aliases":["cerita pendek"],"category":"bahasa","answer":"Cerpen adalah cerita fiksi pendek yang biasanya berfokus pada satu konflik utama, tokoh terbatas, alur ringkas, dan pesan tertentu."},{"topic":"pidato","aliases":["speech writing"],"category":"bahasa","answer":"Pidato adalah penyampaian gagasan secara lisan di depan audiens. Struktur umumnya pembuka, isi, dan penutup, dengan bahasa yang sesuai tujuan dan pendengar."},{"topic":"debat","aliases":["argumentasi debat"],"category":"bahasa","answer":"Debat adalah pertukaran argumen untuk mempertahankan pendapat. Debat yang baik memakai data, logika, etika, dan sanggahan yang relevan."},{"topic":"ringkasan","aliases":["summary","meringkas teks"],"category":"bahasa","answer":"Ringkasan adalah penyajian inti informasi secara lebih pendek tanpa menghilangkan gagasan utama. Ringkasan harus objektif, jelas, dan tidak menambah opini baru."},{"topic":"literasi digital","aliases":["digital literacy"],"category":"bahasa","answer":"Literasi digital adalah kemampuan mencari, menilai, memakai, dan membuat informasi digital secara aman, kritis, etis, dan produktif."},{"topic":"hoaks","aliases":["hoax","berita palsu"],"category":"bahasa","answer":"Hoaks adalah informasi palsu atau menyesatkan yang disebarkan seolah benar. Cara memeriksa hoaks: cek sumber, tanggal, bukti, konteks, dan pembanding tepercaya."},{"topic":"ekonomi mikro","aliases":["microeconomics"],"category":"ekonomi","answer":"Ekonomi mikro mempelajari keputusan individu, rumah tangga, dan perusahaan, termasuk permintaan, penawaran, harga, produksi, dan perilaku konsumen."},{"topic":"ekonomi makro","aliases":["macroeconomics"],"category":"ekonomi","answer":"Ekonomi makro mempelajari perekonomian secara luas, seperti inflasi, pengangguran, pertumbuhan ekonomi, nilai tukar, kebijakan fiskal, dan kebijakan moneter."},{"topic":"inflasi lanjutan","aliases":["inflation explained"],"category":"ekonomi","answer":"Inflasi adalah kenaikan harga barang dan jasa secara umum. Inflasi dapat dipengaruhi permintaan tinggi, biaya produksi naik, pasokan terganggu, atau kebijakan moneter."},{"topic":"deflasi","aliases":["deflation"],"category":"ekonomi","answer":"Deflasi adalah penurunan harga umum secara berkelanjutan. Walau terlihat menguntungkan, deflasi bisa menandakan permintaan lemah dan menekan ekonomi."},{"topic":"permintaan dan penawaran lanjutan","aliases":["supply demand curve"],"category":"ekonomi","answer":"Permintaan menunjukkan keinginan membeli pada berbagai harga, sedangkan penawaran menunjukkan keinginan menjual. Titik pertemuannya membentuk harga keseimbangan."},{"topic":"elastisitas harga","aliases":["price elasticity"],"category":"ekonomi","answer":"Elastisitas harga mengukur seberapa besar perubahan permintaan atau penawaran saat harga berubah. Barang kebutuhan pokok biasanya lebih tidak elastis."},{"topic":"akuntansi dasar","aliases":["basic accounting"],"category":"ekonomi","answer":"Akuntansi dasar mencatat, mengelompokkan, dan melaporkan transaksi keuangan. Tujuannya membantu pemilik, manajer, dan pihak terkait memahami kondisi keuangan."},{"topic":"laporan laba rugi","aliases":["income statement"],"category":"ekonomi","answer":"Laporan laba rugi menunjukkan pendapatan, beban, dan laba atau rugi dalam periode tertentu. Laporan ini membantu menilai kinerja usaha."},{"topic":"neraca","aliases":["balance sheet"],"category":"ekonomi","answer":"Neraca menunjukkan posisi aset, kewajiban, dan ekuitas pada tanggal tertentu. Rumus dasarnya: aset = kewajiban + ekuitas."},{"topic":"arus kas","aliases":["cash flow"],"category":"ekonomi","answer":"Arus kas mencatat uang masuk dan keluar. Usaha bisa untung di laporan, tetapi tetap bermasalah jika arus kas tidak cukup untuk membayar kebutuhan."},{"topic":"pemasaran","aliases":["marketing"],"category":"bisnis","answer":"Pemasaran adalah proses memahami kebutuhan pasar, membuat penawaran bernilai, mengomunikasikan manfaat, dan membangun hubungan pelanggan."},{"topic":"branding","aliases":["merek","brand strategy"],"category":"bisnis","answer":"Branding adalah usaha membangun identitas, persepsi, dan kepercayaan terhadap merek melalui nama, visual, nilai, kualitas, cerita, dan pengalaman pelanggan."},{"topic":"segmentasi pasar","aliases":["market segmentation"],"category":"bisnis","answer":"Segmentasi pasar membagi pasar menjadi kelompok pelanggan dengan kebutuhan atau karakteristik mirip agar strategi produk dan promosi lebih tepat."},{"topic":"customer journey","aliases":["perjalanan pelanggan"],"category":"bisnis","answer":"Customer journey adalah tahapan pengalaman pelanggan dari mengenal merek, mempertimbangkan, membeli, memakai, hingga loyal atau merekomendasikan."},{"topic":"kewirausahaan","aliases":["entrepreneurship"],"category":"bisnis","answer":"Kewirausahaan adalah proses melihat peluang, membuat solusi bernilai, mengambil risiko terukur, mengelola sumber daya, dan membangun usaha berkelanjutan."},{"topic":"manajemen operasi","aliases":["operations management"],"category":"bisnis","answer":"Manajemen operasi mengatur proses produksi atau layanan agar efisien, konsisten, berkualitas, dan mampu memenuhi permintaan pelanggan."},{"topic":"rantai pasok","aliases":["supply chain"],"category":"bisnis","answer":"Rantai pasok adalah aliran barang, informasi, dan uang dari pemasok hingga pelanggan. Manajemen rantai pasok menyeimbangkan biaya, kecepatan, kualitas, dan risiko."},{"topic":"negosiasi","aliases":["negotiation"],"category":"bisnis","answer":"Negosiasi adalah proses mencapai kesepakatan melalui komunikasi, tawar-menawar, pemahaman kepentingan, opsi solusi, dan sikap saling menghormati."},{"topic":"manajemen risiko","aliases":["risk management"],"category":"bisnis","answer":"Manajemen risiko adalah proses mengenali, menilai, mengurangi, memantau, dan merespons kemungkinan kejadian yang dapat mengganggu tujuan."},{"topic":"demokrasi lanjutan","aliases":["sistem demokrasi"],"category":"sosial","answer":"Demokrasi adalah sistem pemerintahan yang memberi rakyat peran dalam keputusan publik melalui pemilu, perwakilan, kebebasan berpendapat, hukum, dan akuntabilitas."},{"topic":"hak asasi manusia","aliases":["ham","human rights"],"category":"sosial","answer":"Hak asasi manusia adalah hak dasar yang melekat pada setiap orang, seperti hak hidup, keamanan, pendidikan, kebebasan berpendapat, dan perlakuan setara."},{"topic":"hukum dasar","aliases":["pengertian hukum"],"category":"sosial","answer":"Hukum adalah aturan yang mengatur perilaku masyarakat dan memiliki sanksi. Tujuannya menciptakan ketertiban, keadilan, perlindungan, dan kepastian."},{"topic":"konstitusi","aliases":["undang undang dasar"],"category":"sosial","answer":"Konstitusi adalah aturan dasar negara yang mengatur lembaga, kekuasaan, hak warga, dan prinsip pemerintahan. Di Indonesia, konstitusinya adalah UUD 1945."},{"topic":"administrasi publik","aliases":["public administration"],"category":"sosial","answer":"Administrasi publik adalah pengelolaan kebijakan, layanan, organisasi, dan sumber daya pemerintah untuk memenuhi kebutuhan masyarakat."},{"topic":"sosiologi","aliases":["sociology"],"category":"sosial","answer":"Sosiologi mempelajari masyarakat, interaksi sosial, kelompok, norma, konflik, perubahan sosial, dan lembaga seperti keluarga, pendidikan, ekonomi, dan negara."},{"topic":"psikologi dasar","aliases":["basic psychology"],"category":"sosial","answer":"Psikologi dasar mempelajari perilaku dan proses mental manusia, seperti persepsi, emosi, motivasi, belajar, ingatan, kepribadian, dan hubungan sosial."},{"topic":"komunikasi interpersonal","aliases":["interpersonal communication"],"category":"sosial","answer":"Komunikasi interpersonal adalah pertukaran pesan antarindividu. Kualitasnya dipengaruhi kejelasan, empati, mendengar aktif, bahasa tubuh, dan umpan balik."},{"topic":"konflik sosial","aliases":["social conflict"],"category":"sosial","answer":"Konflik sosial terjadi ketika kelompok atau individu memiliki kepentingan, nilai, atau sumber daya yang bertentangan. Penyelesaiannya membutuhkan dialog, mediasi, dan keadilan."},{"topic":"globalisasi","aliases":["globalization"],"category":"sosial","answer":"Globalisasi adalah meningkatnya keterhubungan dunia melalui perdagangan, teknologi, budaya, informasi, migrasi, dan kerja sama antarnegara."},{"topic":"peta dan skala","aliases":["map scale","skala peta"],"category":"geografi","answer":"Skala peta menunjukkan perbandingan jarak pada peta dengan jarak sebenarnya. Skala membantu menghitung jarak dan memahami ukuran wilayah."},{"topic":"garis lintang dan bujur","aliases":["latitude longitude"],"category":"geografi","answer":"Garis lintang menunjukkan posisi utara-selatan dari khatulistiwa, sedangkan garis bujur menunjukkan posisi timur-barat dari meridian utama."},{"topic":"benua dan samudra","aliases":["continents oceans"],"category":"geografi","answer":"Benua adalah daratan luas, sedangkan samudra adalah perairan sangat besar. Pemahaman benua dan samudra membantu membaca peta dunia dan hubungan wilayah."},{"topic":"urbanisasi","aliases":["urbanization"],"category":"geografi","answer":"Urbanisasi adalah perpindahan penduduk atau peningkatan aktivitas dari desa ke kota. Dampaknya bisa berupa peluang ekonomi, kepadatan, dan kebutuhan infrastruktur."},{"topic":"proklamasi indonesia","aliases":["proklamasi kemerdekaan","17 agustus 1945"],"category":"sejarah","answer":"Proklamasi Kemerdekaan Indonesia dibacakan pada 17 Agustus 1945 oleh Soekarno didampingi Mohammad Hatta di Jakarta, menandai lahirnya Indonesia sebagai negara merdeka."},{"topic":"kerajaan sriwijaya lanjutan","aliases":["sriwijaya maritim"],"category":"sejarah","answer":"Sriwijaya adalah kerajaan maritim bercorak Buddha yang berpusat di Sumatra dan kuat dalam perdagangan, pelayaran, serta hubungan budaya Asia Tenggara."},{"topic":"kerajaan majapahit lanjutan","aliases":["majapahit nusantara"],"category":"sejarah","answer":"Majapahit adalah kerajaan besar di Jawa yang mencapai puncak pada masa Hayam Wuruk dan Gajah Mada, dengan pengaruh politik dan budaya luas di Nusantara."},{"topic":"kolonialisme","aliases":["colonialism"],"category":"sejarah","answer":"Kolonialisme adalah penguasaan suatu wilayah oleh kekuatan asing untuk kepentingan politik, ekonomi, dan strategis. Dampaknya memengaruhi struktur sosial dan ekonomi banyak negara."},{"topic":"imperialisme","aliases":["imperialism"],"category":"sejarah","answer":"Imperialisme adalah perluasan kekuasaan negara atas wilayah atau bangsa lain melalui politik, ekonomi, militer, atau budaya."},{"topic":"revolusi industri lanjutan","aliases":["industrial revolution"],"category":"sejarah","answer":"Revolusi Industri mengubah produksi manual menjadi produksi mesin, memicu pabrik, urbanisasi, transportasi modern, dan perubahan besar dalam ekonomi serta masyarakat."},{"topic":"perang dunia pertama","aliases":["ww1","perang dunia 1"],"category":"sejarah","answer":"Perang Dunia Pertama berlangsung 1914-1918 dan dipicu ketegangan aliansi, nasionalisme, militerisme, imperialisme, serta pembunuhan Franz Ferdinand."},{"topic":"perang dunia kedua lanjutan","aliases":["ww2","perang dunia 2"],"category":"sejarah","answer":"Perang Dunia Kedua berlangsung 1939-1945 dan melibatkan banyak negara. Dampaknya meliputi perubahan geopolitik, berdirinya PBB, dan awal tatanan dunia baru."},{"topic":"perang dingin","aliases":["cold war"],"category":"sejarah","answer":"Perang Dingin adalah ketegangan ideologi dan geopolitik antara blok Amerika Serikat dan Uni Soviet setelah Perang Dunia Kedua, tanpa perang langsung besar antara keduanya."},{"topic":"reformasi 1998","aliases":["era reformasi indonesia"],"category":"sejarah","answer":"Reformasi 1998 di Indonesia menandai perubahan politik dari Orde Baru menuju era demokrasi lebih terbuka, desentralisasi, kebebasan pers, dan pembaruan kelembagaan."},{"topic":"jalur rempah","aliases":["spice route"],"category":"sejarah","answer":"Jalur rempah adalah jaringan perdagangan rempah yang menghubungkan Nusantara dengan Asia, Timur Tengah, dan Eropa. Rempah bernilai tinggi dan memengaruhi sejarah pelayaran dunia."},{"topic":"renaisans","aliases":["renaissance"],"category":"sejarah","answer":"Renaisans adalah masa kebangkitan seni, ilmu, dan pemikiran di Eropa yang menekankan humanisme, observasi, kreativitas, dan warisan klasik."},{"topic":"revolusi perancis","aliases":["french revolution"],"category":"sejarah","answer":"Revolusi Prancis dimulai 1789 dan mengguncang monarki absolut. Ide kebebasan, persamaan, dan persaudaraan memengaruhi politik modern dunia."},{"topic":"sejarah internet","aliases":["asal usul internet"],"category":"sejarah","answer":"Internet berkembang dari riset jaringan komputer seperti ARPANET, lalu tumbuh menjadi jaringan global untuk komunikasi, informasi, bisnis, pendidikan, dan hiburan."},{"topic":"gizi seimbang","aliases":["balanced nutrition"],"category":"kesehatan","answer":"Gizi seimbang berarti memenuhi kebutuhan energi, protein, lemak, karbohidrat, vitamin, mineral, serat, dan air sesuai kondisi tubuh. Untuk masalah medis pribadi, konsultasikan tenaga kesehatan."},{"topic":"olahraga rutin","aliases":["aktivitas fisik"],"category":"kesehatan","answer":"Olahraga rutin membantu kebugaran jantung, otot, tulang, metabolisme, dan suasana hati. Pilih aktivitas aman sesuai usia dan kondisi, lalu tingkatkan bertahap."},{"topic":"tidur berkualitas","aliases":["sleep hygiene","kualitas tidur"],"category":"kesehatan","answer":"Tidur berkualitas dipengaruhi jadwal teratur, lingkungan nyaman, cahaya redup, membatasi kafein, dan mengurangi layar sebelum tidur. Keluhan berat perlu konsultasi profesional."},{"topic":"stres","aliases":["stress management","manajemen stres"],"category":"kesehatan","answer":"Stres adalah respons tubuh terhadap tekanan. Manajemen stres dapat dilakukan dengan istirahat, napas teratur, olahraga, dukungan sosial, dan menyelesaikan masalah bertahap."},{"topic":"kesehatan mental umum","aliases":["mental wellbeing"],"category":"kesehatan","answer":"Kesehatan mental umum berkaitan dengan emosi, pikiran, hubungan, dan kemampuan menghadapi tekanan. Bantuan profesional penting jika gangguan terasa berat atau berlangsung lama."},{"topic":"pertolongan pertama dasar","aliases":["first aid basic"],"category":"kesehatan","answer":"Pertolongan pertama dasar adalah bantuan awal sebelum tenaga medis datang, seperti memastikan keamanan, memeriksa respons, menghentikan perdarahan ringan, dan mencari bantuan darurat bila perlu."},{"topic":"kebersihan diri","aliases":["personal hygiene"],"category":"kesehatan","answer":"Kebersihan diri mencakup mencuci tangan, mandi, merawat gigi, menjaga pakaian bersih, dan kebiasaan higienis untuk mengurangi risiko penyakit."},{"topic":"vaksinasi umum","aliases":["imunisasi umum"],"category":"kesehatan","answer":"Vaksinasi membantu sistem imun mengenali penyakit tertentu. Jadwal dan kebutuhan vaksin sebaiknya mengikuti rekomendasi tenaga kesehatan dan otoritas kesehatan."},{"topic":"sistem pernapasan lanjutan","aliases":["respiratory system"],"category":"kesehatan","answer":"Sistem pernapasan membawa oksigen ke tubuh dan mengeluarkan karbon dioksida melalui hidung, tenggorokan, trakea, bronkus, paru-paru, dan alveolus."},{"topic":"sistem pencernaan","aliases":["digestive system"],"category":"kesehatan","answer":"Sistem pencernaan memecah makanan menjadi nutrisi yang diserap tubuh. Organ utamanya mulut, kerongkongan, lambung, usus halus, usus besar, hati, pankreas, dan empedu."},{"topic":"jantung dan pembuluh darah","aliases":["cardiovascular system"],"category":"kesehatan","answer":"Jantung memompa darah melalui pembuluh darah untuk mengantar oksigen dan nutrisi. Kesehatan jantung dipengaruhi aktivitas fisik, pola makan, tidur, stres, dan faktor medis."},{"topic":"sistem imun","aliases":["immune system"],"category":"kesehatan","answer":"Sistem imun melindungi tubuh dari kuman dan zat asing. Komponennya meliputi sel darah putih, antibodi, organ limfoid, kulit, dan respons peradangan."},{"topic":"manajemen waktu","aliases":["time management"],"category":"pengembangan diri","answer":"Manajemen waktu adalah kemampuan mengatur prioritas, jadwal, energi, dan fokus agar tugas penting selesai tanpa mengabaikan istirahat."},{"topic":"belajar efektif lanjutan","aliases":["effective learning"],"category":"pengembangan diri","answer":"Belajar efektif memakai tujuan jelas, latihan aktif, pengulangan berjeda, membuat rangkuman, mengerjakan soal, dan memperbaiki kesalahan."},{"topic":"berpikir kritis lanjutan","aliases":["critical thinking advanced"],"category":"pengembangan diri","answer":"Berpikir kritis adalah menilai informasi dengan bukti, logika, konteks, dan kesadaran bias sebelum menyimpulkan atau mengambil keputusan."},{"topic":"pemecahan masalah","aliases":["problem solving"],"category":"pengembangan diri","answer":"Pemecahan masalah dimulai dari memahami masalah, mencari akar penyebab, membuat opsi, menilai risiko, mencoba solusi, dan mengevaluasi hasil."},{"topic":"pengambilan keputusan","aliases":["decision making"],"category":"pengembangan diri","answer":"Pengambilan keputusan yang baik mempertimbangkan tujuan, data, pilihan, konsekuensi, risiko, nilai, dan umpan balik setelah keputusan dijalankan."},{"topic":"kreativitas","aliases":["creativity"],"category":"pengembangan diri","answer":"Kreativitas adalah kemampuan menghasilkan ide baru atau menggabungkan ide lama menjadi solusi berguna. Kreativitas tumbuh melalui eksplorasi, latihan, dan keberanian mencoba."},{"topic":"empati","aliases":["empathy"],"category":"pengembangan diri","answer":"Empati adalah kemampuan memahami perasaan dan sudut pandang orang lain. Empati membantu komunikasi, kerja sama, pelayanan pelanggan, dan penyelesaian konflik."},{"topic":"etika kerja","aliases":["work ethic"],"category":"pengembangan diri","answer":"Etika kerja mencakup tanggung jawab, kejujuran, disiplin, menghargai waktu, menjaga kualitas, berkomunikasi baik, dan menghormati rekan kerja."},{"topic":"produktifitas","aliases":["produktivitas","productivity"],"category":"pengembangan diri","answer":"Produktivitas bukan sekadar sibuk, tetapi menghasilkan hal penting dengan sumber daya dan waktu yang efektif. Fokus, prioritas, dan evaluasi membantu produktivitas."},{"topic":"keuangan pribadi","aliases":["personal finance"],"category":"pengembangan diri","answer":"Keuangan pribadi mencakup mengatur pemasukan, pengeluaran, tabungan, dana darurat, utang, asuransi, dan investasi sesuai tujuan serta risiko masing-masing."},{"topic":"menabung","aliases":["saving money"],"category":"pengembangan diri","answer":"Menabung adalah menyisihkan sebagian penghasilan untuk tujuan masa depan atau dana darurat. Menabung lebih mudah jika dibuat otomatis dan memiliki target jelas."},{"topic":"investasi dasar","aliases":["basic investing"],"category":"pengembangan diri","answer":"Investasi dasar adalah menempatkan dana pada aset dengan harapan nilai berkembang. Setiap investasi memiliki risiko, jadi pahami tujuan, jangka waktu, dan profil risiko."},{"topic":"perubahan iklim","aliases":["climate change"],"category":"lingkungan","answer":"Perubahan iklim adalah perubahan pola suhu dan cuaca jangka panjang. Aktivitas manusia seperti pembakaran bahan bakar fosil meningkatkan gas rumah kaca yang mempercepat pemanasan global."},{"topic":"energi terbarukan","aliases":["renewable energy"],"category":"lingkungan","answer":"Energi terbarukan berasal dari sumber yang dapat pulih alami, seperti matahari, angin, air, panas bumi, dan biomassa. Energi ini membantu mengurangi emisi jika dikelola baik."},{"topic":"daur ulang","aliases":["recycling"],"category":"lingkungan","answer":"Daur ulang mengolah kembali sampah menjadi bahan atau produk baru. Tujuannya mengurangi limbah, menghemat sumber daya, dan menekan pencemaran."},{"topic":"pengelolaan sampah","aliases":["waste management"],"category":"lingkungan","answer":"Pengelolaan sampah mencakup pengurangan, pemilahan, penggunaan ulang, daur ulang, pengomposan, dan pembuangan akhir yang aman."},{"topic":"pencemaran udara","aliases":["air pollution"],"category":"lingkungan","answer":"Pencemaran udara terjadi saat zat berbahaya masuk atmosfer, seperti asap kendaraan, industri, debu, dan pembakaran. Dampaknya bisa mengganggu kesehatan dan lingkungan."},{"topic":"pencemaran air","aliases":["water pollution"],"category":"lingkungan","answer":"Pencemaran air terjadi ketika limbah, bahan kimia, mikroba, atau sampah masuk sungai, danau, laut, atau air tanah sehingga menurunkan kualitas air."},{"topic":"konservasi alam","aliases":["nature conservation"],"category":"lingkungan","answer":"Konservasi alam adalah upaya melindungi spesies, habitat, air, tanah, dan ekosistem agar tetap lestari untuk generasi sekarang dan masa depan."},{"topic":"jejak karbon","aliases":["carbon footprint"],"category":"lingkungan","answer":"Jejak karbon adalah jumlah emisi gas rumah kaca dari aktivitas manusia, produk, perjalanan, makanan, atau energi. Pengurangan dapat dilakukan dengan efisiensi dan pilihan rendah emisi."},{"topic":"pertanian berkelanjutan","aliases":["sustainable agriculture"],"category":"lingkungan","answer":"Pertanian berkelanjutan berusaha menghasilkan pangan sambil menjaga tanah, air, keanekaragaman hayati, kesejahteraan petani, dan ketahanan jangka panjang."},{"topic":"ekonomi sirkular","aliases":["circular economy"],"category":"lingkungan","answer":"Ekonomi sirkular adalah model yang mengurangi limbah dengan mendesain produk agar tahan lama, bisa diperbaiki, dipakai ulang, didaur ulang, atau dikembalikan ke siklus produksi."},{"topic":"analisis swot","aliases":["swot"],"category":"bisnis","answer":"Analisis SWOT menilai Strengths, Weaknesses, Opportunities, dan Threats. Alat ini membantu memahami posisi usaha atau proyek sebelum menyusun strategi."},{"topic":"metode ilmiah lanjutan","aliases":["scientific method"],"category":"sains","answer":"Metode ilmiah adalah proses sistematis: observasi, pertanyaan, hipotesis, eksperimen, analisis data, kesimpulan, dan komunikasi hasil."},{"topic":"hipotesis","aliases":["hypothesis"],"category":"sains","answer":"Hipotesis adalah dugaan sementara yang dapat diuji dengan data atau eksperimen. Hipotesis harus jelas, terukur, dan bisa dibantah bila bukti tidak mendukung."},{"topic":"variabel penelitian","aliases":["research variable"],"category":"sains","answer":"Variabel penelitian adalah hal yang diukur atau dikendalikan dalam studi. Ada variabel bebas, terikat, kontrol, dan pengganggu."},{"topic":"sampel penelitian","aliases":["research sample"],"category":"sains","answer":"Sampel penelitian adalah sebagian anggota populasi yang diteliti. Sampel yang baik mewakili populasi agar kesimpulan lebih dapat dipercaya."},{"topic":"validitas","aliases":["validity research"],"category":"sains","answer":"Validitas menunjukkan apakah alat ukur atau penelitian benar-benar mengukur hal yang seharusnya diukur."},{"topic":"reliabilitas","aliases":["reliability research"],"category":"sains","answer":"Reliabilitas menunjukkan konsistensi hasil pengukuran. Alat ukur reliabel memberi hasil relatif stabil ketika kondisi pengukuran sama."},{"topic":"visualisasi data","aliases":["data visualization"],"category":"teknologi","answer":"Visualisasi data menyajikan informasi dalam grafik, peta, tabel, atau diagram agar pola, perbandingan, dan tren lebih mudah dipahami."},{"topic":"dashboard","aliases":["dashboard data"],"category":"teknologi","answer":"Dashboard menampilkan indikator penting secara ringkas dan visual. Dashboard yang baik fokus pada keputusan, bukan sekadar banyak grafik."},{"topic":"kpi","aliases":["key performance indicator"],"category":"bisnis","answer":"KPI adalah indikator utama untuk mengukur kemajuan terhadap tujuan. KPI harus relevan, terukur, dapat dipantau, dan terkait tindakan."},{"topic":"okr","aliases":["objectives and key results"],"category":"bisnis","answer":"OKR adalah kerangka tujuan yang terdiri dari Objective yang inspiratif dan Key Results yang terukur. OKR membantu fokus dan penyelarasan tim."},{"topic":"lean startup","aliases":["startup lean"],"category":"bisnis","answer":"Lean startup menekankan eksperimen cepat, produk minimum layak, pengukuran, dan pembelajaran agar usaha tidak membuang sumber daya pada asumsi yang salah."},{"topic":"design thinking","aliases":["pemikiran desain"],"category":"bisnis","answer":"Design thinking adalah pendekatan menyelesaikan masalah dengan memahami pengguna, mendefinisikan masalah, membuat ide, prototipe, dan menguji solusi."},{"topic":"agile","aliases":["metode agile"],"category":"teknologi","answer":"Agile adalah pendekatan kerja iteratif yang membagi pekerjaan menjadi siklus pendek, menerima perubahan, dan mengutamakan umpan balik pengguna."},{"topic":"scrum","aliases":["kerangka scrum"],"category":"teknologi","answer":"Scrum adalah kerangka Agile dengan peran, sprint, backlog, daily meeting, review, dan retrospektif untuk mengelola pengembangan produk secara iteratif."}]);
const DIRAC_ULTRA_100K_OPENERS = Object.freeze([
  '', 'Tolong ', 'Mohon ', 'Bisa bantu ', 'Coba ', 'Silakan ', 'Tolong bantu ', 'Mohon bantu ', 'Aku ingin kamu ', 'Saya ingin kamu ',
  'Bantu saya ', 'Bantu aku ', 'Untuk pemula, ', 'Dengan bahasa sederhana, ', 'Secara ringkas, ', 'Dengan contoh, ', 'Tanpa istilah sulit, ', 'Untuk pelajar, ', 'Dalam konteks umum, ', 'Secara praktis, '
]);
const DIRAC_ULTRA_100K_REQUESTS = Object.freeze([
  'jelaskan {topic}.', 'terangkan {topic}.', 'definisikan {topic}.', 'ringkas {topic}.', 'uraikan inti {topic}.',
  'beri pengertian {topic}.', 'beri definisi {topic}.', 'buat penjelasan {topic}.', 'buat rangkuman {topic}.', 'tuliskan pengertian {topic}.',
  'tuliskan definisi {topic}.', 'sebutkan poin penting {topic}.', 'jelaskan konsep dasar {topic}.', 'jelaskan manfaat memahami {topic}.', 'jelaskan contoh penerapan {topic}.',
  'jelaskan cara kerja dasar {topic}.', 'jelaskan fungsi utama {topic}.', 'jelaskan ciri-ciri {topic}.', 'jelaskan perbedaan dasar terkait {topic}.', 'jelaskan hubungan {topic} dengan kehidupan sehari-hari.',
  'jelaskan kenapa {topic} penting.', 'jelaskan gambaran umum {topic}.', 'jelaskan {topic} untuk anak sekolah.', 'buat catatan singkat {topic}.', 'jawab pertanyaan tentang {topic}.'
]);
...
function buildDiracUltra100kQA() {
  ...
  for (const concept of DIRAC_ULTRA_100K_CONCEPTS) {
    for (const pattern of DIRAC_ULTRA_100K_PATTERNS) {
      const question = pattern.replace(/\{topic\}/g, concept.topic).replace(/\s+/g, ' ').trim();
      const key = normalize(question);
      if (!key || exact.has(key)) continue;
      exact.set(key, Object.freeze({ question, answer: concept.answer, topic: concept.topic, category: concept.category }));
      count += 1;
    }
  }
...
const DIRAC_ULTRA_100K_META = Object.freeze({
  ...
  scope: 'pengetahuan umum lintas aspek: teknologi, AI, matematika, sains, bahasa, ekonomi, bisnis, sosial, sejarah, kesehatan umum, lingkungan, dan pengembangan diri',
  preservation: 'SERVER_PRODUCTS dan lapisan pengetahuan produk parfum 50K tidak diubah atau dikurangi'
});
function diracUltra100kCountAnswer(text) {
  const n = normalize(text);
  if (!n) return null;
  if (/\b(parfum|perfume|fragrance|produk|katalog|checkout|keranjang|resi|stok|ready|harga)\b/.test(n)) return null;
  const asksBank = /\b(berapa|jumlah|total)\b.*\b(pertanyaan|qa|qna|knowledge|pengetahuan|ilmu|jawaban)\b|\b(pertanyaan|qa|qna|knowledge|pengetahuan|ilmu|jawaban)\b.*\b(berapa|jumlah|total)\b/.test(n);
...
function diracUltra100kCommerceGuard(text) {
  const n = normalize(text);
  if (!n) return true;
  if (typeof diracUltra10kCommerceGuard === 'function' && diracUltra10kCommerceGuard(n)) return true;
  if (isStoreProductPriceQuestion(n)) return true;
  if (/\b(parfum|perfume|fragrance|produk dirac|katalog dirac|website ini|web ini|toko ini|di katalog|di website|checkout|keranjang|order|pesan|resi|paket|kurir|stok|ready|sold|harga parfum|harga produk|aroma|wangi|notes)\b/.test(n)) return true;
  return false;
}
function diracUltra100kRealtimeGuard(text) {
  const n = normalize(text);
  if (typeof diracUltra10kRealtimeGuard === 'function' && diracUltra10kRealtimeGuard(n)) return true;
  return /\b(hari ini|sekarang|saat ini|terbaru|real time|realtime|live|update terbaru|besok|kemarin|minggu ini|bulan ini|tahun ini)\b/.test(n) && /\b(harga|kurs|cuaca|berita|presiden|menteri|saham|crypto|kripto|bitcoin|jadwal|skor|hasil pertandingan|undang undang terbaru|aturan terbaru)\b/.test(n);
}
function diracUltra100kHasIntent(text) {
  const n = normalize(text);
  return /\b(apa|apa itu|pengertian|definisi|maksud|arti|jelaskan|terangkan|ringkas|rangkuman|materi|pelajaran|contoh|sebutkan|kenapa|mengapa|bagaimana|bantu|ajari|ajarkan|tuliskan|buatkan|poin|konsep|dasar|fungsi|manfaat|ciri|karakteristik|kesimpulan|catatan|uraikan)\b/.test(n);
}
...
function diracUltra100kAnswer(text) {
  const n = normalize(text);
  if (!n || diracUltra100kCommerceGuard(n) || diracUltra100kRealtimeGuard(n)) return null;
  ...
  const concepts = DIRAC_ULTRA_100K_CONCEPTS.slice().sort((a, b) => String(b.topic).length - String(a.topic).length);
  for (const concept of concepts) {
    const phrases = [concept.topic].concat(concept.aliases || []).sort((a, b) => String(b).length - String(a).length);
    for (const phrase of phrases) {
      const normalizedPhrase = normalize(phrase);
      if (normalizedPhrase.length < 3) continue;
      if (diracUltra100kRegexFromPhrase(normalizedPhrase).test(n)) return diracUltra100kFormatAnswer(concept);
    }
  }
  return null;
}
